	...
	#ifndef BISCUIT_PARSER_PP_SHORTEST_INCLUDED
		#define BISCUIT_PARSER_PP_SHORTEST_INCLUDED
		...
		// local macros
		#define BISCUIT_parser_shortest_body_header() \
		...
		#define BISCUIT_parser_shortest_body_elem(z, n, data) \
			if (P##n::parse(s, us)) \
			{ \
				ret = true; \
		...
		#define BISCUIT_parser_shortest_body_footer() \
			if (ret) \
			{ \
				std::advance(s.cur, d); \
			} \
		...
		BOOST_PP_IDENTITY(#include <limits>)()
		...
		struct shortest
		{
			...
			{
				BISCUIT_parser_shortest_body_header()
				
				#define BOOST_PP_LOCAL_MACRO(n)   BISCUIT_parser_shortest_body_elem(~, n, ~)
				#define BOOST_PP_LOCAL_LIMITS     (0, BISCUIT_PARSER_MAX_ARITY-1)
				#include BOOST_PP_LOCAL_ITERATE()
				#undef BOOST_PP_LOCAL_MACRO

				BISCUIT_parser_shortest_body_footer()
			}
		};
		...
		#define BOOST_PP_FILENAME_1 "shortest.hpp"
		#include BOOST_PP_ITERATE()

		} // namespace biscuit
		#undef BISCUIT_parser_shortest_body_header
		#undef BISCUIT_parser_shortest_body_elem
		#undef BISCUIT_parser_shortest_body_footer

	#endif // BISCUIT_PARSER_PP_SHORTEST_INCLUDED
