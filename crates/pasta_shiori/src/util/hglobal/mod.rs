...
impl ShioriString {
    ...
    fn clone_from_slice_impl(bytes: &[u8], has_free: bool) -> ShioriString {
        let len = bytes.len();
        // SAFETY: GlobalAlloc with GMEM_FIXED returns a pointer to a newly allocated
        // block of `len` bytes. from_raw_parts_mut is valid because:
        // - h is non-null (GlobalAlloc succeeds or process terminates)
        // - the allocated region is exactly `len` bytes
