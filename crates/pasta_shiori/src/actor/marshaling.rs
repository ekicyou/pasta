...
//!
//! # GET タイムアウト閾値（R5.8/R5.9）
//! [`GET_TIMEOUT`] 初期値 6.68ms（PoC 申し送り）。通常運転（デバッガ非停止・アクター
//! スレッド正常進行）では実 GET はこの値より十分速く完了するため発火せず、応答バイト列は
//! 不変（R5.8）。タイムアウトは **デバッガ停止中も抑止しない**（R5.9）— 停止中の 204 は
//! 次 `OnSecondChange` でコルーチンが resume され回復するため、デバッグ容易性を損なわない。
//! 実機実測に基づく最終チューニングは RN4（後続）に委ねる。
//!
...
/// GET の block-on-reply タイムアウト閾値（R5.8・初期値 6.68ms・PoC 申し送り）。
///
/// 通常運転では発火しない安全網。デバッガ停止中も抑止しない（R5.9）。実機実測に基づく
/// 最終チューニングは RN4 に委ねる（design.md OPEN QUESTION 3）。
pub const GET_TIMEOUT: Duration = Duration::from_micros(6_680);
