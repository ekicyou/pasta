...
fn test_context_file_attrs_accumulation() {
    ...
    let attr1 = pasta_rune::parser::Attr {
        key: "天気".to_string(),
        value: pasta_rune::parser::AttrValue::AttrString("晴れ".to_string()),
        span: Span::default(),
    };
    let attr2 = pasta_rune::parser::Attr {
        key: "季節".to_string(),
        value: pasta_rune::parser::AttrValue::AttrString("夏".to_string()),
        span: Span::default(),
    };
...
fn test_context_merge_attrs_scene_priority() {
    ...
    // File-level attr
    let file_attr = pasta_rune::parser::Attr {
        key: "季節".to_string(),
        value: pasta_rune::parser::AttrValue::AttrString("冬".to_string()),
        span: Span::default(),
    };
    ctx.accumulate_file_attr(&file_attr);

    // Scene-level attrs override file-level
    let scene_attrs = vec![pasta_rune::parser::Attr {
        key: "季節".to_string(),
        value: pasta_rune::parser::AttrValue::AttrString("夏".to_string()),
        span: Span::default(),
    }];
    ...
    assert_eq!(
        merged.get("季節"),
        Some(&pasta_rune::parser::AttrValue::AttrString("夏".to_string()))
    );
}
...
fn test_transpile_to_string_empty_file() {
    let file = pasta_rune::parser::PastaFile::new(std::path::PathBuf::from("test.pasta"));
