...
fn create_test_word_table() -> pasta_rune::runtime::WordTable {
    let selector = Box::new(pasta_rune::DefaultRandomSelector::new());
    let registry = pasta_rune::WordDefRegistry::new();
    pasta_rune::runtime::WordTable::from_word_def_registry(registry, selector)
}
...
fn test_dynamic_error_from_rune_script() {
    ...
    let selector = Box::new(pasta_rune::DefaultRandomSelector::new());
    let table = pasta_rune::SceneTable::new(selector);
    let word_table = create_test_word_table();

    let result = pasta_rune::stdlib::create_module(table, word_table);
    assert!(
        result.is_ok(),
...
fn test_error_type_parse_error() {
    // Test PastaError::Parse construction (wraps ParseError::SyntaxError)
    ...
    match &err {
        ...
        _ => panic!("Expected Parse(SyntaxError) variant"),
    }
