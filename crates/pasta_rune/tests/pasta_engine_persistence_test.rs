...
"#;
    ...
    let talk_event = events
        .iter()
        .filter(|e| matches!(e, pasta_rune::ScriptEvent::Talk { .. }))
        .last();
    if let Some(pasta_rune::ScriptEvent::Talk { content, .. }) = talk_event {
        let text = content
            .iter()
            .filter_map(|p| {
                if let pasta_rune::ir::ContentPart::Text(t) = p {
                    Some(t)
                } else {
                    None
                }
...
"#,
    ...
    if let Some(pasta_rune::ScriptEvent::Talk { content, .. }) = load_events
        .iter()
        .filter(|e| matches!(e, pasta_rune::ScriptEvent::Talk { .. }))
        .last()
    {
...
"#;
    ...
    let get_path = |events: &[pasta_rune::ScriptEvent]| {
        let talk_event = events
            .iter()
            .filter(|e| matches!(e, pasta_rune::ScriptEvent::Talk { .. }))
            .last();
...
"#,
    ...
    if let Some(pasta_rune::ScriptEvent::Talk { content, .. }) = events
        .iter()
        .filter(|e| matches!(e, pasta_rune::ScriptEvent::Talk { .. }))
        .last()
    {
