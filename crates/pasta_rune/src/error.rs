//! Error types for Pasta Rune runtime layer.
//!
//! This module defines runtime errors for the Rune language backend.
//! Parse errors are defined in pasta_core and can be converted to PastaError.

use pasta_core::error::{ParseError, SceneTableError, WordTableError};
...
#[derive(Error, Debug)]
pub enum PastaError {
    /// Parse error (forwarded from pasta_core).
    #[error(transparent)]
    Parse(#[from] ParseError),

    /// Scene table error (forwarded from pasta_core).
    #[error(transparent)]
    SceneTable(#[from] SceneTableError),

    /// Word table error (forwarded from pasta_core).
    #[error(transparent)]
    ...
    MultipleParseErrors {
        errors: Vec<pasta_core::error::ParseErrorInfo>,
    },
...
impl PastaError {
    ...
    pub fn parse_error(
        ...
        PastaError::Parse(ParseError::SyntaxError {
            file: file.into(),
            line,
            column,
