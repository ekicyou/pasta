...
use pasta_core::registry::SceneTable;
use pasta_core::registry::WordTable;
...
fn word_expansion(word_table: &Mutex<WordTable>, module_name: String, key: String) -> String {
    ...
    match table.search_word(&module_name, &key, &[]) {
        ...
        Err(e) => {
            ...
            match &e {
                pasta_core::WordTableError::WordNotFound { key: word_key } => {
                    warn!(
                        "単語定義 @{} が見つかりません（モジュール: {}）",
                        word_key,
...
mod tests {
    ...
    fn test_create_module() {
        ...
        use pasta_core::{DefaultRandomSelector, WordDefRegistry};
