...
use crate::error::{PastaError, Result, Transpiler2Pass};
use crate::ir::ScriptEvent;
use crate::loader::{DirectoryLoader, ErrorLogWriter};
use crate::runtime::{DefaultRandomSelector, RandomSelector, SceneTable, WordTable};
use crate::transpiler::Transpiler2;
use pasta_core::parser::{self, FileItem, PastaFile};
...
impl PastaEngine {
    ...
    pub fn with_random_selector(
        ...
        let mut parse_errors: Vec<pasta_core::ParseErrorInfo> = Vec::new();
        ...
        for pasta_file in &loaded.pasta_files {
            match parser::parse_file(pasta_file) {
                ...
                Err(e) => {
                    // Convert parse error to ParseErrorInfo
                    match &e {
                        pasta_core::ParseError::SyntaxError {
                            file,
                            line,
                            column,
                            message,
                        } => {
                            parse_errors.push(pasta_core::ParseErrorInfo {
                                file: file.clone(),
                                line: *line,
                                column: *column,
