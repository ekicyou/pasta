...
fn containment_single_scene_resolves_that_scene() {
    // global シーン 会話1 が 行 10..=20（end_line=20）。
    let mut b = idx();
    b.add_scene("a.pasta", "会話1", 10, 20, 0);
    let index = b.finish();

    // 範囲内のクリック → 会話1。
    assert_eq!(index.scene_at("a.pasta", 10).as_deref(), Some("会話1"));
    assert_eq!(index.scene_at("a.pasta", 15).as_deref(), Some("会話1"));
    assert_eq!(index.scene_at("a.pasta", 20).as_deref(), Some("会話1"));
}

#[test]
fn innermost_local_preferred_over_enclosing_global() {
    // global 会話1: 行 10..=40。その中に local 会話1_挨拶: 行 20..=30。
    let mut b = idx();
    b.add_scene("a.pasta", "会話1", 10, 40, 0);
    b.add_scene("a.pasta", "会話1_挨拶", 20, 30, 1);
    let index = b.finish();

    // local 範囲外（global のみ）→ global。
    assert_eq!(index.scene_at("a.pasta", 15).as_deref(), Some("会話1"));
    // local 範囲内（両方包含）→ 最内 local 優先。
    assert_eq!(index.scene_at("a.pasta", 20).as_deref(), Some("会話1_挨拶"));
    assert_eq!(index.scene_at("a.pasta", 25).as_deref(), Some("会話1_挨拶"));
    assert_eq!(index.scene_at("a.pasta", 30).as_deref(), Some("会話1_挨拶"));
    // local 範囲後・global 範囲内 → global へ戻る。
    assert_eq!(index.scene_at("a.pasta", 35).as_deref(), Some("会話1"));
}
...
fn backward_fallback_to_nearest_scene_at_or_below() {
    ...
    let mut b = idx();
    b.add_scene("a.pasta", "会話1", 10, 20, 0);
    b.add_scene("a.pasta", "会話2", 30, 40, 0);
    let index = b.finish();

    // ヘッダ（行 5）→ 下方最近接 会話1。
    assert_eq!(index.scene_at("a.pasta", 5).as_deref(), Some("会話1"));
    // 宣言行ちょうど（行 10）は包含で 会話1。
    assert_eq!(index.scene_at("a.pasta", 10).as_deref(), Some("会話1"));
    // シーン間の隙間（行 25・どこにも包含されない）→ 下方最近接 会話2。
    assert_eq!(index.scene_at("a.pasta", 25).as_deref(), Some("会話2"));
}
