...
fn test_regex_match_captures() {
    ...
    assert!(result.is_ok());
    let value = result.unwrap();
    assert_eq!(value_as_str(&value).unwrap(), "2024");
}
...
fn test_regex_named_captures() {
    ...
    assert!(result.is_ok());
    let value = result.unwrap();
    assert_eq!(value_as_str(&value).unwrap(), "01");
}
...
fn test_regex_replace() {
    ...
    assert!(result.is_ok());
    let value = result.unwrap();
    assert_eq!(value_as_str(&value).unwrap(), "hello world");
}
...
fn test_regex_shortcut_match() {
    ...
    assert!(result.is_ok());
    let value = result.unwrap();
    assert_eq!(value_as_str(&value).unwrap(), "test@example.com");
}
...
fn test_regex_escape() {
    ...
    assert!(result.is_ok());
    let value = result.unwrap();
    assert_eq!(value_as_str(&value).unwrap(), "a\\*b\\+c\\?");
}
...
fn test_regex_unicode_word_boundary() {
    ...
    assert!(result.is_ok());
    let value = result.unwrap();
    assert_eq!(value_as_str(&value).unwrap(), "世界です");
}
