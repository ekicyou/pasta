...
6. [@pasta_sakura_script モジュール](#6-pasta_sakura_script-モジュール)
7. [pasta.finalize_scene 関数](#7-pastafinalize_scene-関数)
8. [mlua-stdlib 統合モジュール](#8-mlua-stdlib-統合モジュール)
9. [SHIORI EVENT ハンドラ](#9-shiori-event-ハンドラ)
...
| `@pasta_sakura_script` | スクリプト変換系   | —          | テキストからさくらスクリプトへの変換        |
...
## 6. @pasta_sakura_script モジュール

テキストからさくらスクリプトへの変換機能を提供するモジュールです。セリフテキストに自然な会話テンポを演出するためのウェイトタグ（`\_w[ms]`）を自動挿入します。

```lua
local SAKURA_SCRIPT = require "@pasta_sakura_script"
```

### 6.1 talk_to_script(actor, talk)

テキストにウェイトタグを挿入してさくらスクリプトに変換します。
...
| パラメータ | 型     | 必須 | 説明                                                        |
| ---------- | ------ | ---- | ----------------------------------------------------------- |
| `actor`    | table  | ✅    | actor オブジェクト。`talk` サブテーブルにウェイト設定を持つ |
| `talk`     | string | ✅    | 変換対象のセリフテキスト                                    |

#### actor.talk テーブルのフィールド

| フィールド                | 型      | デフォルト     | 説明                             |
| ------------------------- | ------- | -------------- | -------------------------------- |
| `script_wait_default`     | integer | 50             | デフォルトのウェイト値（ms）     |
| `script_wait_period`      | integer | 100            | 句点「。」のウェイト値（ms）     |
| `script_wait_comma`       | integer | 75             | 読点「、」のウェイト値（ms）     |
| `script_wait_newline`     | integer | 100            | 改行のウェイト値（ms）           |
| `script_wait_exclamation` | integer | 100            | 感嘆符・疑問符のウェイト値（ms） |
...
ウェイトタグが挿入されたさくらスクリプト文字列。

#### 動作仕様

1. **さくらスクリプトタグの保護**: 既存のタグ（`\s[5]`, `\_w[100]` など）はそのまま保持
...
local actor2 = {
    talk = {
        script_wait_default = 80,
        script_wait_period = 200,
    }
}
...
script_wait_newline = 100
script_wait_exclamation = 100
...
これらの設定は `PastaConfig.talk()` 経由で取得され、actor の `talk` テーブルとマージされます。actor 側の設定が優先されます。

---

## 7. pasta.finalize_scene 関数
