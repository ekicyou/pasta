...
- 四者の役割分離を明確化（`doc/spec/` → `references/` → `SKILL.md`、`GRAMMAR.md` は独立）
...
|-----------|-------------|--------|--------------|------------------|-----------|
| SKILL.md §1 修正 | スキル/メイン | 役割分離＋情報権威フロー明記 | 4.3, 4.5, 6.1, 6.2 | — | — |
...
| Field | Detail |
|-------|--------|
| Intent | 四者の役割分離と情報権威フローを明記 |
| Requirements | 4.3, 4.5, 6.1, 6.2 |

**Responsibilities & Constraints**
- 情報権威フロー `doc/spec/` → `references/` → `SKILL.md` を明記
- `GRAMMAR.md` がフローとは独立であることを明記
...
変更後 §1（追加部分）:
```markdown
**役割分離**: 本スキルは以下の情報権威フローに基づく:
- `doc/spec/`（権威仕様書）→ `references/`（AI向け再構成）→ `SKILL.md`（要約＋パターン集）
- `GRAMMAR.md`（人間向け学習用）はこのフローとは独立した文書であり、AI向け情報チェーンには含まれない
- SKILL.md と `references/` の記述に矛盾がある場合、`references/`（権威文書のAI向け再構成）を正とする

**自己完結性**: 本スキルは別リポジトリにコピーして単体で機能する。
`doc/spec/` や `GRAMMAR.md` への外部参照に依存しない。
```
...
> **情報源**: `doc/spec/XX-xxx.md` をAI向けに再構成。
> 権威は `doc/spec/` にある。本ファイルはLLMによるコード生成に最適化した再構成版。
```

**共通制約**:
- `doc/spec/` の完全コピーではなく、AI向けに再構成（表、コード例、ルール要約を重視）
...
|-----------|---------|-----------|---------|
| §1 Purpose | 8行 | 18行 | 役割分離＋情報権威フロー追加 |
