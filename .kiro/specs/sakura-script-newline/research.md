...
3. **改行の挿入位置（微細仕様）**: 再登場スポットの手番が先頭に非テキストトークン（`surface`/`wait`）を含む場合、`\n[N]` を `\p[spot]` 直後（=先頭トークンの前）に置くか、最初の一般文字列の直前まで遅延するか。requirements.md R2.3 は「`\p[spot]` の後」を暫定採用。
4. **再登場だが可視文字ゼロの手番**: has-text 済みスポットへ戻る手番が本ビルドでは surface のみ（可視文字を出さない）場合、R2.1 の素直な適用は再びゴミ改行（`\p[0]\n[150]\s[5]` で終端）を生む可能性。改行を「切替先スポットの過去 has-text」だけでなく「当該手番が実際に一般文字列を出すこと」でも条件付けるか（完全 lazy=次テキスト直前フラッシュ）を要判断。
5. **同一スポットを共有する複数アクター**: 2 アクターが同一 spot ID を持つ場合の切替（eager は `last_spot == spot` で改行抑制）を、lazy でどう扱うか（同一バルーン内の段落区切りとして改行するか否か）。
