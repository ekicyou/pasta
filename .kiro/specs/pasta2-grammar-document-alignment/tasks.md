...
- [ ] 1.1 1.3節「式の制約（設計方針）」を「式（Expression）のサポート」に改名し、式の正式採用を記載
...
- [ ] 2.1 9.1節「変数型」を拡張し、ローカル変数とグローバル変数の有効範囲を正確に記載
...
- [ ] 3.1 4.1節「Call ターゲットの形式」を更新し、グローバルシーン参照パターンを削除
...
- [ ] 4.1 「式の制約（重要）」セクション全体を「式（Expression）のサポート」に置換
...
- [ ] 5.1 変数スコープテーブル（line 221-225）を更新し、有効範囲を正確に記載
  - ローカル変数の有効範囲を「親シーン内」→「一連のシーンが終わるまで」に変更
  - グローバル変数の有効範囲を「ファイル全体」→「永続的」に変更
  - _Requirements: 3.4, 5.2_

- [ ] 5.2 変数参照セクション（line 228-235）を確認し、有効範囲説明が反映されているか検証
...
- [ ] 6.1 (P) Call ターゲットの形式テーブル（line 278-297）を更新し、グローバルシーン行を削除
...
- [ ] 6.2 (P) GRAMMAR.md 内で `@*id` または `>*id` の記載がないか確認
  - grep検索で `@\*`, `>\*` を検索
...
- [ ] 8.1 (P) 修正後の SPECIFICATION.md が pasta2.pest と一致することを目視確認
...
- [ ] 8.2 (P) 修正後の GRAMMAR.md が pasta2.pest と一致することを目視確認
...
- [ ] 9.1 修正後の comprehensive_control_flow2.pasta が pasta2.pest でエラーなくパース可能か確認
...
- [ ] 10.1 (P) SPECIFICATION.md で廃止構文（Jump文、全角\\、全角[]）の説明が残存していないか確認
  - grep検索で `？`, `全角\\`, `全角[]` を検索
...
- [ ] 11.1 (P) GRAMMAR.md で廃止構文（Jump文、全角\\、全角[]）が記載されていないか確認
  - grep検索で `？`, `全角\\`, `全角[]` を検索
