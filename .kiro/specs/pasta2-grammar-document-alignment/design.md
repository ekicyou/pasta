...
#### 修正2: 2.3節「変数」にグローバル変数説明追加

**現状**: ローカル変数のみ説明
**pasta2.pest**: `var_ref_global = { var_marker ~ global_marker ~ id ~ s }` を定義
**修正**: グローバル変数サブセクションを追加

```markdown
### 2.3.3 グローバル変数

グローバル変数はセッション中有効な変数です。
...
```pasta
＄＊グローバル変数＝100  # グローバル変数代入
Alice：値は ＄＊グローバル変数 です  # グローバル変数参照
```
```

#### 修正3: Call文セクションの説明確認

**現状**: call_scene_global の説明がある可能性
**pasta2.pest**: `call_scene_global` は存在しない（削除済み）
**修正**: グローバルCall（`>*id`）の説明を削除、ローカル・グローバル兼用の `call_scene` のみ説明

```markdown
### 2.4 Call文

Call文はシーンを呼び出し、実行後に復帰します。

| 構文 | pasta2.pest規則 | 説明 |
|------|----------------|------|
| Call | `call_scene = { call_marker ~ id ~ s ~ args? }` | `＞シーン名` または `＞シーン名（引数）` |

**注意**: グローバルCall（`>*id`）は廃止されました。シーン検索はローカル・グローバル兼用です。
```
...
#### 修正3: グローバル機能参照の説明を削除（グローバル単語定義は保持）

**現状**: グローバル機能参照（`@*id`, `>*id`）の説明がある可能性
**pasta2.pest**: `word_ref_global`（`@*id`）、`call_scene_global`（`>*id`）は存在しない
**保持対象**: グローバル単語定義（`@単語キー`）は有効（pasta2.pestで定義）
**修正**: グローバル機能参照（`@*id`, `>*id`）の説明のみ削除。グローバル単語定義セクションは保持
