...
- [ ] 1. SetValue列挙型をast.rsに定義する
- [ ] 1.1 SetValue列挙型の定義と導出トレイトの実装 (P)
...
- [ ] 1.2 VarSet構造体のvalue フィールド型をExprからSetValueに変更 (P)
...
- [ ] 2. parse_var_set関数をword_ref対応に更新する
- [ ] 2.1 parse_var_set関数内のword_ref検出ロジックを実装 (P)
...
- [ ] 2.2 parse_var_setの修正後、ユニットテスト（パーサー層）を追加・修正 (P)
...
- [ ] 3. トランスパイラー層（generate_var_set関数）をSetValue対応に更新する
- [ ] 3.1 generate_var_set関数をSetValueのパターンマッチに対応させる
...
- [ ] 4. 統合テスト（parser2_integration_test.rs）を修正・実行する
- [ ] 4.1 既存統合テストをSetValue対応に修正
...
- [ ] 4.2 統合テスト追加：word_ref パース・トランスパイル
...
- [ ] 5. 最終確認：cargo check/test全スイート実行
- [ ] 5.1 ビルド・テスト成功確認
