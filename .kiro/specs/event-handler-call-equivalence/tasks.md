...
- [x] 1. `act:find_scene()` の抽出と `act:call()` へのリファクタリング

- [x] 1.1 `act.lua` に `ACT_IMPL.find_scene()` メソッドを実装する
...
- [x] 1.2 `act:call()` の内部実装を `find_scene()` ベースに書き換える
...
- [x] 2. (P) `SCENE.co_exec()` のシグネチャ変更と `find_scene` への委譲
...
- [x] 3. イベントディスパッチ呼び出し元の一斉更新（タスク 2 完了後）

- [x] 3.1 (P) `EVENT.no_entry()` の `SCENE.co_exec()` 呼び出しを新シグネチャに更新する
...
- [x] 3.2 (P) `REG.OnBoot` デフォルトハンドラの `SCENE.co_exec()` 呼び出しを更新する
...
- [x] 3.3 (P) 仮想ディスパッチャの `create_scene_thread()` 内の `SCENE.co_exec()` 呼び出しを更新する
...
- [x] 6. ドキュメント整合性の確認と更新
  - 実装完了後、以下のドキュメントとの整合性を確認・更新する：
    1. [x] SOUL.md — コアバリュー・設計原則との整合性確認（コードパス1本化原則に沿った変更であることを確認）
    2. [x] `doc/spec/` — 言語仕様への影響確認（今回は内部リファクタリングのみにつき変更不要の可能性が高い）
    3. [x] TEST_COVERAGE.md — `act:find_scene()` 単体テスト・GLOBAL フォールバック統合テストの追加をマッピング
    4. [x] `crates/pasta_lua/README.md` — `act:find_scene()` API の追加を反映（該当する場合）
    5. [x] `.agents/skills/pasta-lua-coding/` — `act:find_scene(key, scope?, attrs?)` の API ドキュメントと解決優先順位（L1〜L5）の説明を追加する（Req 1.4）
    6. [x] `steering/*` — 該当領域のステアリング更新
  - _Requirements: 1.4_
