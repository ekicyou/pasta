...
1. When pasta ランタイムが初期化されたとき, the `pasta.global` module shall `GLOBAL.チェイン` 関数と `GLOBAL.yield` 関数を提供する。
2. The `GLOBAL.チェイン` function and `GLOBAL.yield` function shall `act:yield()` を実行し、蓄積されたトークンを中間出力として返す（両者は同一動作）。
...
### Requirement 2: ランタイム動作試験 — `＞チェイン` から `GLOBAL.チェイン` への解決
...
1. When Pasta DSL に `　＞チェイン` を含むシーンをトランスパイルして実行したとき, the runtime shall `GLOBAL.チェイン` 関数を呼び出す。
2. When `GLOBAL.チェイン` が呼び出されたとき, the runtime shall `act:yield()` を実行し、蓄積トークンを中間出力として返す。
...
### Requirement 3: インテグレーションテスト — `EVENT.fire` 経由の yield 挙動確認
...
1. When `＞チェイン` を1回含むシーンが `EVENT.fire` 経由で実行されたとき, the runtime shall コルーチンを2回 resume する（yield 前の出力 + yield 後の出力）。
