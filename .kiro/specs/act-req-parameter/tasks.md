...
- [x] 3.1 (P) boot.lua のハンドラを function(act) に変更
  - `REG.OnBoot = function(req)` → `function(act)` に変更
  - req 参照箇所を `act.req` に変更（存在する場合）
  - _Requirements: 2.1_

- [x] 3.2 (P) second_change.lua のハンドラを function(act) に変更
...
- [x] 4.1 M.dispatch のシグネチャを function(act) に変更
  - 引数を `(req, act)` から `(act)` に変更
  - req が必要な箇所は `act.req` から取得（イベント名、日時情報等）
  - _Requirements: 2.2_

- [x] 4.2 execute_scene, check_hour, check_talk を act ベースに変更
