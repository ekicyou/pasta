...
2. **言語**: 日本語のみ（多言語対応は Out of Scope）
...
| GRAMMAR.md    | 古い情報の更新、実装との乖離解消、リンク追加のみ           | Low    |
| steering/*.md | AGENTS.md からの参照追記                                   | Low    |

**互換性評価**:
- ✅ 既存リンク維持（パス変更なし）
- ✅ 後方互換性保証（既存参照者への影響なし）
- ✅ GRAMMAR.md は現状維持のため既存リンクへの影響なし
...
**組み合わせ戦略** (優先順位: D→C→B→A):

| フェーズ | アクション             | Option     | 対象要求 | 優先度 |
| -------- | ---------------------- | ---------- | -------- | ------ |
| Phase 1  | 相互参照整備           | A          | Req 6    | **D**  |
| Phase 2  | AGENTS.md 再構成       | A          | Req 3, 5 | **C**  |
| Phase 3  | ルートREADME拡充       | A          | Req 3, 7 | **B**  |
| Phase 4  | クレートREADME新規作成 | B          | Req 4    | **A**  |
| Phase 5  | 保守ガイドライン作成   | B または A | Req 8    | -      |

**段階的実装** (優先順位 D→C→B→A に基づく):

**Phase 1: 相互参照整備** (Req 6 = 優先度D)
- 全ドキュメントのクロスリファレンス追加
- 孤立ドキュメント解消（内部ドキュメントを除く）
- GRAMMAR.md → SPECIFICATION.md 参照強化
- 既存ドキュメント間のリンク網を構築

**Phase 2: AGENTS.md 再構成** (Req 3, 5 = 優先度C)
- ステアリング参照セクション明示
- Kiro仕様駆動開発ワークフローの詳細化
- steering/*.md との相互リンク
- AI開発支援コンテキストの最適化

**Phase 3: ルートREADME拡充** (Req 3, 7 = 優先度B)
- ドキュメントマップセクション追加
- オンボーディングパス（ユーザー/開発者/AI向け）追加
- クイックスタート追加
- プロジェクト全体のエントリーポイント強化

**Phase 4: クレートREADME作成** (Req 4 = 優先度A)
- `pasta_core/README.md` 作成（pasta_lua/README.md をテンプレート）
- `pasta_shiori/README.md` 作成
- ルートREADMEから各クレートREADMEへのリンク追加

**Phase 5: 保守ガイドラインとアーカイブ整理** (Req 8)
...
**リスク軽減**:
- ✅ フェーズ毎に検証可能（段階的レビュー）
- ✅ Phase 1完了時点で Req 6 達成（優先度D：相互参照網構築による情報アクセス改善）
- ✅ 各フェーズが優先順位順に実施（開発者要望に沿った段階的価値提供）
...
開発者との議論により、以下の設計方針が確定：

| #   | 判断事項                 | 確定方針                           | 理由                                                         |
| --- | ------------------------ | ---------------------------------- | ------------------------------------------------------------ |
| 1   | オンボーディングパス配置 | **(A) README.md 内にセクション追加** | ファイル数増加を抑制、エントリーポイント集約                 |
| 2   | 保守ガイドライン配置     | **(A) workflow.md に「ドキュメント保守」セクション追加** | 既存ワークフローに統合、AI参照容易性向上                     |
| 3   | GRAMMAR.md 対応方針      | **(C) 現状維持（整理と乖離解消のみ）** | 仕様変更の可能性が高く、現時点では整理と実装との乖離解消に注力 |

**GRAMMAR.md 方針の詳細**:
- 大幅な削減や構造変更は行わない（仕様変更リスクが高いため）
- SPECIFICATION.md との重複は許容（両方を独立した完全ドキュメントとして保持）
- 作業範囲: 古い情報の更新、実装との乖離解消、リンク追加のみ
