...
会話スピードを自然に表現するために、テキストに適切なさくらスクリプトウェイトを付与したい。`@pasta_sakura_script`rustモジュールを公開し、lua側からウェイトなしテキストとパラメーターを指定することで、ウェイト設定したさくらスクリプトを取得したい。
...
**Objective:** As a ゴースト開発者, I want Lua側から`@pasta_sakura_script`モジュールを呼び出してウェイト付きスクリプトを取得できること, so that 会話テキストに自然なテンポを付与できる

#### Acceptance Criteria

1. When `require "@pasta_sakura_script"` を呼び出したとき, the pasta_lua shall `talk_to_script` 関数を含むモジュールテーブルを返す
2. When `talk_to_script(actor, talk)` を呼び出したとき, the pasta_sakura_script shall actorテーブルからウェイト設定を読み取り、talk文字列にウェイトを挿入したスクリプト文字列を返す
3. If actorテーブルがnilまたは無効な場合, then the pasta_sakura_script shall pasta.tomlのデフォルト設定を使用してウェイトを挿入する
...
1. When actorテーブルにウェイト設定キーが存在するとき, the pasta_sakura_script shall 以下のキーからウェイト値（ミリ秒）を取得する:
...
1. The pasta_sakura_script shall 入力文字列を以下のトークン種別に分解する:
   ...
   - `general`: 上記以外の一般文字
2. The pasta_sakura_script shall トークン分解時にUnicode文字を正しく処理する
3. The pasta_sakura_script shall さくらスクリプトタグを最優先でマッチングし、タグ内部を分解しない
...
1. If talk文字列がnilまたは空の場合, then the pasta_sakura_script shall 空文字列を返す
2. If トークン分解中にエラーが発生した場合, then the pasta_sakura_script shall 元のtalk文字列をそのまま返し、警告ログを出力する
3. If ウェイト値の計算結果がオーバーフローする場合, then the pasta_sakura_script shall 安全な最大値（i32::MAX）を使用する
...
1. When 入力が `」」」！？。、` でscript_wait_periodが1000の場合, the pasta_sakura_script shall `」」」！？。、\_w[950]` を出力する（最大ウェイト1000から50を引いた値）
2. When 入力が `こんにちは` でscript_wait_normalが100の場合, the pasta_sakura_script shall 各文字の後に`\_w[50]`を挿入する
3. When 入力が `\h\s[0]こんにちは` の場合, the pasta_sakura_script shall さくらスクリプト`\h\s[0]`をそのまま保持し、後続の文字にのみウェイトを挿入する
