...
- [x] 1.1 (P) SceneRegistryを新規registryモジュールに移動
...
- [x] 1.2 (P) WordDefRegistryを新規registryモジュールに移動
...
- [x] 3.1 TranspileContext2の基本構造定義
  - ローカル/グローバル関数リスト管理
  - 現在のモジュール名追跡
...
- [x] 6.2 重複シーンのランダム選択メカニズム
  - 同名シーン定義の検出とID複数登録
  - scene_selector関数での候補シャッフル
  - pasta::call()ラッパーでの選択実行
  - _Requirements: 3_
  - _Note: 基本構造実装済み。P1機能（ランダム選択）は`pasta_stdlib::select_scene_to_id`に委譲_

---

### 7. テストフィクスチャ作成

- [x] 7.1 (P) 基本シーン変換テストフィクスチャ
