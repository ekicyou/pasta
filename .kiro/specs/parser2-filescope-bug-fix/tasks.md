...
- [ ] 1. FileItem 列挙型とPastaFile構造体を定義
- [ ] 1.1 (P) FileItem列挙型を定義し、3つのバリアント（FileAttr、GlobalWord、GlobalSceneScope）を実装
...
- [ ] 1.2 (P) PastaFile 構造体に items フィールドを追加し、旧フィールドを廃止
...
- [ ] 2. build_ast 関数を修正し、複数ファイルアイテムの累積処理を実現
- [ ] 2.1 (P) build_ast の Rule::file_scope ケースを修正
...
- [ ] 2.2 (P) build_ast の Rule::global_scene_scope ケースを修正
...
- [ ] 2.3 パーサーループで出現順序を保持確認
  - テスト: 複数 file_scope と global_scene_scope が交互出現する場合、items の順序がファイル記述順と一致することを確認
  - _Requirements: 3.3, 1.5_

### Phase 3: 既存テスト移行

- [ ] 3. tests/parser2_integration_test.rs の既存テストを新AST構造に対応させ
- [ ] 3.1 test_file_scope_default テストを修正
...
- [ ] 3.2 test_global_scene_parsing テストを修正
  - 廃止: `file.global_scenes[0]` への直接アクセス
  - 実装: `file.global_scene_scopes()[0]` でアクセス
  - _Requirements: 4.4_

- [ ] 3.3 他のテスト4箇所を一括修正（src/parser2/mod.rs 内のテスト）
...
- [ ] 4. 複数ファイルアイテム出現シナリオをカバーする統合テストを追加
- [ ] 4.1 複数 file_scope 単純連続パターンのテストを追加
...
- [ ] 4.2 file_scope と global_scene_scope 交互出現パターンのテストを追加
...
- [ ] 5. PastaFile.items API が transpiler2 での順次処理に対応していることを確認
- [ ] 5.1 items イテレーション処理の型安全性テストを追加
  - テスト: transpiler2 風の順次処理シミュレーション
  - テスト: items.iter() でループし、match で各バリアントを正確に判定・処理
  - _Requirements: 5.1, 5.3_

- [ ] 5.2 Span情報の伝播確認テストを追加
...
- [ ] 6. PastaFile と FileItem に docコメント・移行ガイドを追加
- [ ] 6.1 FileItem 列挙型にドキュメントコメントを追加
  - 内容: grammar.pest 対応関係、3つのバリアントの説明、使用例
  - _Requirements: 6.2, 6.3_

- [ ] 6.2 PastaFile 構造体と items フィールドにドキュメントコメントを追加
  - 内容: 旧 file_scope / global_scenes フィールドからの移行ガイド、ヘルパーメソッド説明
  - _Requirements: 6.2, 6.4_

- [ ] 6.3 src/parser2/mod.rs のモジュールコメントに準拠性を明記
  - 追記: 「parser2 は grammar.pest `file = ( file_scope | global_scene_scope )*` 仕様に完全準拠」
  - _Requirements: 6.3, 6.4_

- [ ] 6.4 既存 transpiler（legacy parser 使用）への影響ゼロを確認・文書化
...
- [ ] 7. 全体統合テストを実行し、リグレッション・品質指標を確認
- [ ] 7.1 全 parser2 テストを実行（cargo test --lib parser2）
  - 目標: 新規テスト含め全テスト合格（リグレッション0件）
  - _Requirements: 4.1, 4.2, 4.3_

- [ ] 7.2 テストカバレッジが 80% 以上であることを確認
