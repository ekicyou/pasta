...
- ⨉ ~~タイムアウトの 500 能動送信は SHIORI プロトコル上不可能~~ → 設計ディスカッション B3 で解決。sweep は `OnSecondChange` ハンドラ内で実行されるため、そのレスポンスとして 500 を返すのはプロトコル上合法。`OnSecondChange` は毎秒発火するため1回の 500 は実害なし
...
| R3  | OnSecondChange レスポンス消費 | sweep の 500 でディスパッチスキップ、毎秒発火のため実害なし          |
