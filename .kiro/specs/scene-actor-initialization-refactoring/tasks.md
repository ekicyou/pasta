...
- [ ] 1.1 (P) `generate_local_scene()` メソッドのアクター初期化ブロック出力順序を変更
...
- [ ] 1.2 (P) `set_spot()` 出力形式を新しいAPI形式に変更
...
- [ ] 3.1 (P) 既存テストを新形式に合わせて修正（複数/単数アクター）
...
- [ ] 3.2 (P) 既存テストを新形式に合わせて修正（アクター非存在）
...
- [ ] 3.3 (P) 既存テストを新形式に合わせて修正（明示的番号付き）
