...
## ランゲージサーバーの実装
vscodeなどで利用できるように、pasta_lsp（名称は適当です、適切な名前に変更して）クレートを実装したい。要件として、vscodeで実行可能なwasmビルドができること。将来的にpasta_vscodeクレートにてvscodeに提供する予定であるため。ランゲージサーバーとして対応したいのは、まずはvscodeで*.pastaを色分けシンタックス表示したい。将来的にはデバッグ対応までしたいですが、今は仕様に含めなくてよい。
