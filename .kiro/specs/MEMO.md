...
## reqテーブルをact.varに展開してアクセスしやすくしたい
reqテーブルを良い感じに参照して、たとえば、どこがクリックされたのかを判断する。varに展開できれば良いのだが、全イベントで無条件に展開すると処理負担が大きい気がするので、act:XXX関数とかを作ってvarテーブルへの展開処理を実装する。時刻のvar展開と同じような処理。
