...
6. The リファクタリング shall `src/` 配下のインラインテスト（`#[cfg(test)] mod tests`）を原則として `tests/` ディレクトリに外部化する
7. If ファイルのテストがprivateフィールドへの直接アクセスを構造的に必要とする場合, the リファクタリング shall 当該ファイルのインラインテスト維持を例外として記録する
