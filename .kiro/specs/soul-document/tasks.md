...
- [x] 3.1 (P) コメント行パーステスト ✅
  ...
  - トランスパイル結果にコメントテキストが含まれないことを検証
  - _Requirements: 7.1_

- [x] 3.2 (P) 属性継承テスト ✅
  - `test_attribute_inheritance()`を実装
  - ファイルレベル属性がシーンに継承されることを確認
  - 属性付きシーンが正常にトランスパイル・実行可能
  - _Requirements: 7.1_

- [x] 3.3 (P) 変数スコープ完全テスト ✅
  - `test_variable_scope_complete()`を実装
  - ローカル（`＄`）→`var.変数名`、グローバル（`＄＊`）→`save.変数名`の分離を確認
  - 生成されたLuaコードがコンパイル・実行可能
  - _Requirements: 7.1_

- [x] 3.4 (P) エラーメッセージ詳細性テスト ✅
  - `test_error_message_specificity()`を実装
  - 不正構文に対して行番号付きエラーが返ることを確認
  - エラーメッセージが10文字以上の詳細を含むことを検証
  - _Requirements: 7.1_
