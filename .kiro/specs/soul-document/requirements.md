...
3. The test-implementation shall **pasta_shiori失敗テスト（5テスト）を修正**し、Phase 0完了基準を達成する：
...
4. When 新規テストを追加した場合, the test-implementation shall TEST_COVERAGE.mdのマッピングを更新する
5. The test-implementation shall SOUL.md Section 5.6（Phase 0完了基準）の未達成項目を完全達成する：
   - pasta_shiori 100%パス
   - 未テスト領域の特定と受容判断
6. Where テスト実装が困難な領域がある場合, the test-implementation shall 「受容理由」をドキュメント化し、Phase 1以降への延期を判断する
