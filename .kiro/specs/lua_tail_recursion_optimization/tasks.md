...
- [ ] 1. 末尾呼び出し判定ロジックの実装
- [ ] 1.1 (P) `is_callable_item` ヘルパー関数を実装
  - `LocalSceneItem` enum に対する `matches!` マクロベースの判定関数
  - コメントで将来の拡張方法（`FnCall` など）を明示
  - テスト可能な単純な型判定ロジック
  - _Requirements: 1, 5_

- [ ] 1.2 `generate_local_scene_items` メソッドを拡張
...
- [ ] 2. return 文の条件付き生成
- [ ] 2.1 `generate_call_scene` メソッドを拡張
...
- [ ] 2.2 (P) `generate_local_scene_items` 内の `generate_call_scene` 呼び出しを更新
  - メソッド呼び出しに `is_tail_call` フラグを渡す
  - 末尾判定結果を正確に反映
  - _Requirements: 2_

- [ ] 3. 既存テストの互換性確認と新規テストケース追加
- [ ] 3.1 既存テスト実行とリグレッション確認
