...
#### Acceptance Criteria
1. When サンプルゴーストの設定ファイルが読み込まれた場合, the pasta_shiori shall `talk_interval_min` を60（秒）として認識する
2. When サンプルゴーストの設定ファイルが読み込まれた場合, the pasta_shiori shall `talk_interval_max` を90（秒）として認識する
...
3. The テストスイート shall サンプルゴーストの設定ファイルに `talk_interval_min=60` / `talk_interval_max=90` が含まれることを検証する
