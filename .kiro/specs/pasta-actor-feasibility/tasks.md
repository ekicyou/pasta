...
- 8.1（バイト不変検証）: **出荷物 `pasta.dll`（pasta_shiori cdylib）の正規化 sha256 が 1.1 ベースラインと完全一致**（クリーン feature-off release を独立再現、pre-actor-poc ソース `1e94b34a` でも同一 `adae6b39…`）＝R7.2 成立。rlib の whole-file sha はシンボルテーブル/crate SVH fingerprint によりセッション間で**非決定**（pre-actor-poc ソースでも committed baseline と不一致）ため、`capture_baseline.ps1 -Mode verify` は **authoritative（pasta.dll、exit code 駆動）＋informational（rlib、報告のみ）**に分割済み。正規化は link-metadata 20 バイト（COFF TimeDateStamp/CheckSum/DebugDir timestamps×3/RSDS GUID）のみゼロ埋め＝`.text`/`.rdata` は逐語ハッシュなので実コード変更は検出する（hex 1 文字改竄で exit 1 を実証）。8.3 撤去後の復帰確認も同 verify（pasta.dll authoritative）を使うこと。
