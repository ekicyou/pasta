...
アクション行の関数呼び出し実装では、**Rune側の関数定義が存在すること**が必須です。

#### Call文テスト（既存パターン）
- `pasta::call`を使用するため、Rune側の関数定義依存なし
- トランスパイラー出力が正しいタプル形式なら、既存テストで検証可能

#### アクション行関数呼び出しテスト（新規パターン）
- `＠関数()` → `for a in 関数(ctx, (args)) { yield a; }`
  - **必須**: テストフィクスチャ内に`fn 関数(ctx, args) { ... }`を定義
- `＠＊関数()` → `for a in super::関数(ctx, (args)) { yield a; }`
  - **必須**: グローバル関数定義`pub fn 関数(ctx, args) { ... }`をRuneブロックに追加

### テスト検証フロー

| テスト段階 | 検証内容 | 責務 |
|-----------|--------|------|
| 1. トランスパイラー検証 | 出力がタプル形式か（括弧と末尾カンマ） | `cargo test --all` |
| 2. Rune構文検証 | 生成Runeコードがコンパイル可能か | fixtures/*.rn |
| 3. 関数定義依存チェック | 呼び出す関数がfixtures内に定義されているか | 手動確認 + テスト設計 |
...
1. **テストフィクスチャの拡張**: `tests/fixtures/*.pasta` に以下を追加
   - アクション行関数呼び出しのサンプル（`＠関数()` 形式）
   - グローバル関数呼び出しのサンプル（`＠＊関数()` 形式）

2. **Runeブロック定義**: 上記フィクスチャに対応する関数実装
   ```rust
   ＠.rune
   fn 関数(ctx, args) { ... }
   pub fn グローバル関数(ctx, args) { ... }
   ```

3. **ランタイム検証**: Rune VM上でコンパイル・実行確認
   - 生成Runeコードが正常にコンパイルされること
   - 関数呼び出しが正しく解決されること（ローカル優先 / グローバル明示）
