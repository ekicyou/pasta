...
1. Call/Jump文の引数リスト（`pasta::call`関数呼び出し）- シーンセレクター経由
2. アクション行の関数呼び出し（`＠関数（引数）`構文）- Rune関数の直接呼び出し

**Call/Jump文とアクション行の違い**:
- **＞関数名（引数）**: `pasta::call`経由でシーンとして呼び出す（現状維持、タプル化のみ）
- **＠関数（引数）**: Rune関数として直接呼び出す（新規実装、バグ修正）
- すべてのシーン関数は`(ctx, args)`形式なので、どちらも引数を受け取れる
