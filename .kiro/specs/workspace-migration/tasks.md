...
- [x] 1. ルート Cargo.toml をワークスペース構成に変更
...
- [x] 5. レジストリモジュールを pasta_core に移動
...
- [x] 6. ランタイム検索テーブルを pasta_core に抽出
...
- [x] 9. (P) トランスパイラモジュールを pasta_rune に移動
...
- [x] 10. (P) ランタイムモジュール（Rune固有部）を pasta_rune に移動
...
- [x] 11. (P) Engine, Cache, Loader, IR を pasta_rune に移動
...
- すべての要件（9/9）がタスク（17個）にマッピング
- すべてのコンポーネント（14/14）が設計から実装タスクに反映
- タスク依存関係が明確で循環なし
- 各タスクは 1-3 時間程度の作業量
- 並列実行可能なタスク（Phase 3）に `(P)` マーク付与
- テスト・検証タスク含まれている
- 2 階層構造（大タスク + サブタスク不要、番号付けシンプル）
