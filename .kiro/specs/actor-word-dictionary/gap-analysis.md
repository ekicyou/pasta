...
1. **前方一致検索の実装方式**: 現在の `word.lua` には前方一致検索がない。Lua側での実装か、Rust FFI経由か？
2. **フォールバック検索の6レベル**: 現在は4レベル検索。シーン単語とグローバル単語の統合方式
3. **コードブロック言語判定**: `actor_scope` 内の `code_block` は常に Lua と仮定するか、言語識別子を見るか？
