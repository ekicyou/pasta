...
- [ ] 1.1 (P) ActorScope構造体にcode_blocksフィールドを追加
...
- [ ] 1.2 (P) parse_actor_scope関数でcode_blockルールをパース
...
- [ ] 2.1 (P) generate_actor関数でLua配列形式の単語定義を出力
...
- [ ] 2.2 (P) generate_actor関数でcode_blocksを展開
  - code_blocks配列を反復処理
...
- [ ] 2.3 (P) WordDefRegistryにregister_actor()メソッドを追加
...
- [ ] 3.5 PROXY:word()メソッドで6レベルフォールバック検索を実装
...
- [ ] 4.1 テストフィクスチャcomprehensive_fallback_test.pastaを作成
...
- [ ] 4.5 関数優先テスト（T12: オーバーライド）
  - T12: さくら.word("天気") → "アクター関数の天気"（同名関数が辞書より優先）
  - _Requirements: 4_

- [ ] 4.6 エッジケーステスト（T13-T15）
