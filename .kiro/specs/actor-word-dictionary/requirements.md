...
6. When 完全一致で関数が見つかった場合, the Pasta Runtime shall 関数を実行し戻り値を返す
7. When 完全一致で関数以外のnil以外の値が見つかった場合, the Pasta Runtime shall その値をそのまま返す（配列なら最初の要素）

#### 検索優先順位

| 優先度 | 検索対象 | マッチ方式 | 値の解決 |
|--------|----------|------------|----------|
| 1 | アクター `ACTOR[key]` | 完全一致 | 関数→実行、値→そのまま |
| 2 | アクターの単語辞書 | 前方一致 | 配列からランダム選択 |
| 3 | シーン `SCENE[key]` | 完全一致 | 関数→実行、値→そのまま |
| 4 | シーンの単語辞書 | 前方一致 | 配列からランダム選択 |
| 5 | グローバル `GLOBAL[key]` | 完全一致 | 関数→実行、値→そのまま |
| 6 | グローバルの単語辞書 | 前方一致 | 配列からランダム選択 |

**注**: L5グローバルは `pasta.global` モジュールにユーザーが直接Luaで定義する。本仕様では検索ロジックのみ提供し、定義方法はスコープ外とする。
