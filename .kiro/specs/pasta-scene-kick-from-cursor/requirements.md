...
#### Acceptance Criteria
1. When エンジンが位置 (uri, line) を受け取る, the Pasta エンジン shall ロード済みソースマップ／シーン同一性索引を用いて当該位置が属するシーンを確定する。
2. While カーソル位置が複数のシーン（global と内包する local）に該当する, the Pasta エンジン shall 最内シーン（local 優先）を選択する。
...
5. If 受け取った位置がどのシーンにも属さない（シーン宣言の外・シーン間の空行等）, then the Pasta エンジン shall 【要件ディスカッションで確定（OPEN QUESTION 2）。暫定: 最近接の直前シーンを選択せず、シーン未検出として扱い、作者に「カーソル下にシーンがありません」を提示し、再生を行わない】。
...
#### Acceptance Criteria
1. The VSCode Pasta 拡張 shall コンテキストメニュー項目「▶ シーンを実行」を、対象ファイルが `.pasta` 言語である場合に表示する。
2. While Pasta デバッグセッションが接続されていない, when 作者が「▶ シーンを実行」を選択する, the VSCode Pasta 拡張 shall 【要件ディスカッションで確定（OPEN QUESTION 1）。暫定: 「Pasta デバッグセッションが接続されていません」を警告表示し、再生要求を送らない】。
3. If シーン実行要求がエンジンからエラー応答を受け取る, then the VSCode Pasta 拡張 shall 失敗理由を作者へ提示する。
...
2. While ディスク上の `.pasta` がロード済み内容より新しい（未リロード／未保存のため行がずれ得る）, when 作者がシーン実行を要求する, the Pasta システム shall 【要件ディスカッションで確定（OPEN QUESTION 4）。暫定: ロード済みソースマップでベストエフォート解決し、解決できた場合は再生する。staleness の自動検知・リロード要求・警告表示の要否と方式は設計フェーズで判断する】。
3. If ロード済みソースマップで位置がどのシーンにも解決できない, then the Pasta システム shall シーン未検出として扱い（Requirement 2.5 に従う）、誤ったシーンの再生を行わない。
