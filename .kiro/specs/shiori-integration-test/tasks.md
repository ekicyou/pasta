...
- [x] 1. ゴースト定義の修正
- [x] 1.1 (P) boot.pasta の OnBoot シーン削減
...
- [x] 1.2 (P) pasta.toml への [talk] セクション追加
...
- [x] 1.3 pasta_sample_ghost のリビルド
...
- [x] 2. テストユーティリティの拡張
- [x] 2.1 (P) copy_sample_ghost_to_temp 関数の実装
...
- [x] 3.3 OnBoot リクエストのテスト実装
  ...
  - 200 OK 応答を検証
  - Value ヘッダーの存在を確認
  - スポット切り替えタグ（\p[0], \p[1]）の検証
  - 表情変更タグ（\s[n]）の検証
  - ウェイトタグ（\_w[）の検証（pasta.toml [talk] セクション反映確認）
  - テキスト内容（OnBoot挨拶テキスト）の検証
  - _Requirements: 5.1, 5.2, 5.3, 5.4, 5.5, 5.6_

- [x] 4. テストの実行と検証
- [x] 4.1 インテグレーションテストの実行
