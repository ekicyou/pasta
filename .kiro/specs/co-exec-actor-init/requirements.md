...
**Objective:** ゴースト作者として、`actor_spots` にスコープが設定されていないアクター名が `BUILDER.build` で参照されたとき、デフォルト値（0）へのフォールバックが発生したことをログで確認できるべき。これにより、設定漏れやタイプミスによる意図せぬスポット割り当てを早期検出できる。

#### Acceptance Criteria

1. When `BUILDER.build` 内で `actor_spots[actor_name]` が `nil` となりデフォルト値（0）にフォールバックしたとき, pasta shall `@pasta_log` の `warn` レベルで、該当アクター名とフォールバック先スポット値をログ出力する
2. The pasta runtime shall `％` 行の有無自体をパースエラーや警告としない（省略は合法であり、スコープ継承として処理される）

> **統合経緯**: 旧 AC 2.1（毎トークのスコープ継承 debug ログ）は `％` 省略が通常運用であるためノイズとなる。実際の障害点（`actor_spots` フォールバック発動時）に絞った warn ログに集約。旧 AC 2.2（未定義アクター参照警告）はスコープ未設定のケースに包含されるため統合。
