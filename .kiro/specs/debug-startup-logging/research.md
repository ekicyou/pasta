...
1. **R1 と R2 の発火関係**: バインド失敗時、R1（有効化検知）は出る／R2（待ち受け）は出ない。この「R1 だけ出る」挙動を要件 R1.1（有効化状態で起動処理に入る）の意図として明文化するか。
2. **`local_addr` の `Option` 処理**: 成功経路では必ず `Some`。`if let Some(addr)` で握る（防御的）か、契約上 `expect`/`unwrap` 可とするか。ログ目的なら `if let Some` が無難。
3. **ログ文言**: 識別可能な固定文言（R1.3）＋秘密情報非混入（R1.4/R2.4。出すのは loopback `host:port` のみ）。具体文言を design で確定。
