...
1. **シャッフル動作検証** (3.1):
   - フィクスチャ: アクター辞書に 3 値の単語を定義（例: `＠通常：\s[0]、\s[100]、\s[200]`）
   - `set_word_selector()` でモックセレクタを注入し、決定論的に順序を制御
   - **`PROXY_IMPL.word()` 経由で**シーン内からアクター単語を参照し、3 値すべてが消費されることを検証
   - これにより Level 1 短絡（`actor[key]` への不正書き込み再導入）もリグレッションとして検知可能
   - 予想される呼び出し: `lua.load("ACT:word('通常')").eval()` 等、アクタープロキシを経由する呼び出し
