...
- [x] 1. (P) ログファイル名を pasta.log に固定する
...
- [x] 2. (P) tracing_init モジュールを作成する
- [x] 2.1 reload::Layer で subscriber を初期化する API を実装する
...
- [x] 2.2 フィルター動的更新 API を実装してエクスポートする
...
- [x] 3. (P) トランスパイル失敗でロードを中止する
- [x] 3.1 (P) PartialTranspileError の表示にファイルパスを含める
...
- [x] 3.2 .lua ファイル処理失敗を failures に収集する
...
- [x] 3.3 failures 非空時に Err を返してロードを中止する
...
- [x] 4. PastaLoader に Stage 1.5 を組み込む
...
- [x] 5. PastaShiori にエラー保持と早期初期化を実装する
- [x] 5.1 load エラーを保持して request() に伝搬する
...
- [x] 5.2 load() 先頭に Stage 1 早期ロガー初期化を実装する
...
- [x] 6.3 PastaShiori の load 失敗 → request エラー伝搬を確認する
