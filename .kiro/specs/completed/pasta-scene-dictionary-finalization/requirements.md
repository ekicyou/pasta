...
**⚠️ 仕様分割禁止 (No Specification Split Allowed)**

本仕様はスコープが広範囲に及ぶが、以下の理由により**仕様分割は許容できない**：

1. **テスト依存性**: SearchContext構築は、シーン辞書と単語辞書の両方が揃って初めてテスト可能
2. **アーキテクチャ整合性**: トランスパイラのRust側レジストリ廃止とLua側収集は一体の変更
3. **統合リスク**: 部分実装では既存機能が破損し、後続実装までテスト不可能な状態が継続

**設計・実装フェーズでの指示**:
- すべての要件（Req1～Req9）を単一実装サイクルで完成させること
- 「Phase 1で一部実装、Phase 2で残り」のような分割提案は行わないこと
- 実装規模が大きい場合は、実装順序の工夫（レイヤー単位での順次完成）で対応すること
