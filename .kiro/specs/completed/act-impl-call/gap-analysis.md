...
### 決定1: SCENE.search第3引数追加方式 (2026-01-28)

**決定**: Option A - 単純な第3引数追加

**理由**:
- Luaの引数システムにより既存の2引数呼び出しとの互換性が保たれる
- 最小限の変更で将来の拡張に対応可能
- オーバーエンジニアリングを避ける
...
function SCENE.search(name, global_scene_name, attrs)
    -- attrs は将来対応のため現在は無視（nilチェック不要）
    -- 既存ロジックはそのまま
end
```

### 決定2: 実装アプローチ選択 (2026-01-28)

**決定**: Option A - 既存ファイル拡張（ACT_IMPL.call直接置き換え）

**理由**:
- callでやることは自明であり、追加の関数を挟むメリットが限定的
- 最小限の変更で要件を満たせる
- actor.luaの多段検索パターンを流用可能

**補足**:
- テスト容易性のため、検索ロジックを`find_handler`ヘルパー関数として分離することは設計フェーズで検討可
- 基本方針は直接置き換えだが、テスト設計により柔軟に対応

### 決定3: pasta_shiori互換実装の同期戦略 (2026-01-28)

**決定**: Option A - 同時更新

**対象ファイル**:
- `crates/pasta_lua/scripts/pasta/act.lua` (本体実装)
- `crates/pasta_shiori/tests/support/scripts/pasta/act.lua` (テスト用互換実装)
