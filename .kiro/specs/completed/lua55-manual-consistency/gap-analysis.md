...
| **Req 2**: メタデータ統一 | 2形式混在 | blockquote→HTMLコメント形式に変換 | Medium |
| **Req 3**: 6章セクション重複 | 31見出し（本来11） | パート分割を解消し単一見出しに | High |
| **Req 4**: README目次化 | 役割曖昧 | README.mdに詳細目次統合、index.md→API_INDEX.md or 削除 | High |
| **Req 5**: 目次・索引リンク検証 | アンカーID不一致の可能性 | 6章修正後に全リンク検証必要 | High |
...
### 決定済み事項
- **メタデータ形式**: HTMLコメント形式に統一（HTML変換時に非表示）
- **ナビゲーションリンク形式**: パターン2（詳細版）に統一
  - 形式: `[← 前へ: N-1 – タイトル](前.md) | [目次](./README.md) | [次へ: N+1 – タイトル →](次.md)`
  - 理由: 章タイトルを含むことでユーザーが目的地を把握しやすい
  - 影響: 01章の簡易版も詳細版に修正、05-09章に追加する際はこのパターンを使用
- **6章アンカーID命名規則**: 正規化ID形式（シングルハイフン）
  - 形式: `#62-基本関数`（not `#62--基本関数`）
  - 理由: よりクリーンで標準的、日本語マニュアルとして見出しとIDが直接対応しメンテナンス性が高い
  - 影響: index.md等の既存リンクを更新する必要あり（Phase 3で対応）
- **ファイル構成**: ファイル役割逆転方針
  - README.md ← index.mdの詳細目次+索引を移植（GitHubデフォルト表示）
  - ABOUT.md ← 現 README.mdの概要・翻訳方針・免責事項を移動
  - index.md → 削除
  - 理由: GitHubでREADME.mdがデフォルト表示されるため、詳細目次がすぐにアクセス可能
  - 影響: Phase 5でファイル移動と内容統合を実施
