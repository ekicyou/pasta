...
以下の判断フローに従って、各完了済み仕様を評価します：

```mermaid
flowchart TD
    Start[完了済み仕様を1つ選択] --> Q1{COMPLETION_REPORT.md<br/>に実装完了の記載あり?}
    
    Q1 -->|Yes| Q2{現在のコードと<br/>要件が整合?}
    Q1 -->|No or 不明| Delete[削除候補]
    
    Q2 -->|Yes| Q3{Phase 1-4で<br/>参照した?}
    Q2 -->|No, 矛盾| Delete
    
    Q3 -->|Yes| Keep[保持]
    Q3 -->|No| Q4{情報が最新?<br/>参考価値あり?}
    
    Q4 -->|Yes| Keep
    Q4 -->|No| Delete
    
    Delete --> List[削除リストに追加]
    Keep --> Next[次の仕様へ]
    List --> Next
```
...
| 条件                                             | 判定     | 理由                                           |
| ------------------------------------------------ | -------- | ---------------------------------------------- |
| COMPLETION_REPORT.md に「実装完了」記載あり      | 保持     | 完了した仕様は参考価値あり                     |
| COMPLETION_REPORT.md なし or 未完了              | 削除候補 | 実装されていない古い計画                       |
| 要件が現在のコードと矛盾                         | 削除候補 | 混乱を招く旧仕様                               |
| Phase 1-4 でドキュメント作成時に参照             | 保持     | 有用な参照情報                                 |
| 情報が古く、現在の実装と大きく異なる             | 削除候補 | 実装とかけ離れた旧設計                         |
| 完了レポートあり、かつ現在も関連性の高い機能     | 保持     | 将来の仕様作成時の参考資料                     |
| Phase 1-4 で未参照、かつ情報が古い               | 削除候補 | 今後の仕様作成に混乱を招く可能性               |
| 完了レポートに「差し戻し」「再設計中」などの記載 | 削除候補 | 無効化された仕様（要確認後、削除して問題なし） |

**削除実行手順**:
1. 上記フローで全完了済み仕様を評価
2. 削除候補リストを作成（仕様名と削除理由を明記）
3. 開発者に削除リスト提示、最終確認
4. 承認された仕様をgit操作で削除
5. コミットメッセージに削除理由を記載

**Implementation Notes**
- 削除前に必ず開発者確認（判断フローで「削除候補」でも最終判断は開発者）
- 削除はgit操作のため復元可能
- 完了レポート（COMPLETION_REPORT.md）がある仕様は原則保持、ただし要件矛盾は削除候補
