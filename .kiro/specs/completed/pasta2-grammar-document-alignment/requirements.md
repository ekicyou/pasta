...
5. When GRAMMAR.mdの単語定義と参照セクションを読む場合、the GRAMMAR.md shall pasta2.pestのword_ref, key_words（スコープ兼用）構文に従った例を提供する ✅ **[議題A決定] @単語はスコープ兼用（グローバル単語定義も使用可）。@*id（スコープ明示）は削除済み**
