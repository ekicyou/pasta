...
- [x] 1.1 (P) WordEntry 構造体とレジストリの基本実装
...
- [x] 1.5 `WordDefRegistry::all_entries()` でエントリ一覧を取得
  - テストと `WordTable::from_word_def_registry()` での利用を可能に
  - _Requirements: 5.5_

### 2. WordTable の実装（ランタイム層）

- [x] 2.1 (P) WordTable 構造体と RadixMap インデックスの実装
...
- [x] 4.1 (P) `word(module_name, key, filters) -> String` の実装
...
- [x] 4.3 `WordTable` への Mutex ラッピングと登録
...
- [x] 5.4 エンドツーエンド統合テスト
  ...
  - **Note**: 6 tests in `tests/pasta_word_definition_e2e_test.rs`

- [x] 5.5 GRAMMAR.md への単語定義セクション追加
  ...
  - **Note**: GRAMMAR.md Section 7 expanded with full documentation

- [x] 5.6 サンプルスクリプト作成（3 つ以上）
  - グローバル単語定義サンプル
  - ローカル単語定義と前方一致複合検索サンプル
  - グローバル/ローカル混合参照サンプル
  - _Requirements: 8.9_
  - **Note**: Created 3 sample scripts:
    - `examples/scripts/dic/07_word_definition_basic.pasta`
    - `examples/scripts/dic/08_word_definition_prefix_search.pasta`
    - `examples/scripts/dic/09_word_definition_scope.pasta`

- [x] 5.7 引用符エスケープの動作確認（ドキュメント化）
  - 例：`「「test」」` → `「test」` の動作をドキュメント化
  - パーサー層で既に実装済みであることを確認
  - _Requirements: 8.8_
  - **Note**: Documented in GRAMMAR.md "引用符エスケープ" section
