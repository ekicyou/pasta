...
- [x] 5. Core: VSCode UI 動線（カーソル実行・リロード・旧動線撤去）
- [x] 5.1 カーソル位置からのシーン実行コマンドと右クリック動線を実装する
...
- [x] 5.2 SHIORI リロードコマンドと自動再アタッチを実装する
...
- [x] 6.2 トランスポート往復と kick セマンティクス継承を検証する
...
- [x] 6.4 VSCode UI の検証を行う（自動 E2E ＋ 手動 SSP ゲート）
...
- 2.x join_key 契約（**設計の緊張を解決**: design.md:341 の2引数 trait と task 2.1「終端と入れ子レベルを受け渡す」の不整合を、design.md SSOT 優先で解決）：
  ...
  - **2.2 への注意（レビュー指摘 #6・2.1 由来でなく既存性質）→ 解決済み（follow-up）**: `increment_counter` は **raw** `scene.name` をキーにしていたが、ランタイム counter は **sanitized** `base_name` をキーにするため、「異なる raw 名が同一 base へ正規化される」ケース（例 `会話·A`(U+00B7) と `会話_A` がともに `会話_A` へ）で join_key 出現順とランタイム連番がズレ、2 本目の衝突シーンが 1 本目へ**誤解決**していた（scene-kick の join に加え scene 内 word の module_name・local parent_counter にも波及する横断バグ）。**根本修正**: `SceneRegistry::increment_counter`（`scene_registry.rs`）を **sanitized 名キー**へ変更し module_name/join_key/parent_counter を一括整合（非衝突ケースはバイト不変・全ワークスペース緑で検証）。特性化テスト `sanitize_colliding_globals_resolve_to_distinct_runtime_identities`（`tests/scene_identity_index_test.rs`・fixture `scene_identity_collision.pasta`）で固定。`scene_join.rs` は upstream で join_key が正しくなるため無改修。
