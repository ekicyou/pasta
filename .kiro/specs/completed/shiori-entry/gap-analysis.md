...
### 議題1: 移行方式（完全置き換え vs 段階的移行）

**選択肢**:
- A. 段階的移行（main.lua を一時的にラッパーとして残す）
- B. 完全置き換え（scripts/pasta/shiori/main.lua を即座に削除）

**決定**: 選択肢B（完全置き換え）を採用（確定日: 2026-01-27）

**理由**: 
- 設計の明確性を最優先とする
- scripts/ 配下は本番コードのみとする原則を守る
- main.lua は EVENT.fire を呼んでおらず、保持する価値が低い

### 議題2: テストフィクスチャの整理方針

**選択肢**:
- A. tests/fixtures/ 配下は現状維持（main.luaのまま）
- B. 全テストフィクスチャを entry.lua にリネーム

**決定**: 選択肢B（全テストフィクスチャを entry.lua にリネーム）を採用（確定日: 2026-01-27）

**理由**: 
- scripts/ 配下は完全に本番コードのみとし、テストコードとの明確な分離を実現
- tests/ 配下のファイルは全て entry.lua に統一することで、混乱を防ぐ

### 議題3: README.md更新の扱い

**選択肢**:
- A. 要件に含める（Requirement 8として明記）
- B. タスク分解時に個別タスクとして扱う

**決定**: 選択肢A（要件に含める）を採用（確定日: 2026-01-27）

**理由**: 
- エントリーポイント名称変更は外部仕様に影響する重要な変更
- ドキュメント更新も要件として明示すべき
- `scripts/README.md` は開発者が最初に参照する可能性が高い
