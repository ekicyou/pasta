...
- [x] 1.1 (P) シーン関数のパラメータを `ctx` から `act` に変更
...
- [x] 1.2 (P) `PASTA.create_session()` 呼び出しを `act:init_scene()` に置き換え
...
- [x] 1.3 `generate_local_scene()` 関数のドキュメント更新
...
- [x] 2.1 (P) `PASTA.clear_spot(ctx)` を `act:clear_spot()` に置き換え
  - L268 のメソッド呼び出し形式を更新
  - 呼び出しから `ctx` パラメータを削除
...
- [x] 2.2 (P) `PASTA.set_spot()` 呼び出しを `act:set_spot()` 形式に置き換え
  - L270 のメソッド呼び出し形式を更新
...
- [x] 3.1 (P) L510 で word() 呼び出しを talk() でラッピング
...
- [x] 3.2 (P) L362 と L472 で word() 引数に StringLiteralizer を適用
...
- [x] 4.1 (P) L509 でアクター範囲のさくらスクリプトを `act:sakura_script()` に置き換え
...
- [x] 5.2 アクタープロキシとさくらスクリプトのテストアサーション更新
...
- [x] 6.2 code_generator.rs モジュールドキュメント更新
