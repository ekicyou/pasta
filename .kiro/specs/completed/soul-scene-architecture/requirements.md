...
この4要素の関係性は、pastaの実行モデルの本質である。シーンは静的な台本であり、それを実行（「アクション！」）することでアクターが動き出し、actというカメラに向かって演技（会話や表情変化）が記録される。シーン関数は基本的にシナリオ情報をactに出力（撮影）していく流れとなる。

本改訂により、「辞書」（静的データ構造）と「Phase 0」（開発状況）の間に独立章「5. シーン実行アーキテクチャ」（動的実行モデル）を挿入し、この映画撮影メタファーを明確に文書化する。既存の第5章以降は1つずつ繰り下げられる。
...
**Objective:** pastaプロジェクトの設計思想を理解したい開発者として、シーン・アクター・アクション・actの4要素関係を映画撮影メタファーで理解したい。これにより、pastaの実行モデルの本質を直感的に把握できる。

#### Acceptance Criteria

1. 新セクションは、pastaを「台本DSL」として位置づけ、映画撮影メタファーを明示すること
2. 新セクションは、シーン（台本）・アクター（役者）・アクション（演技）・act（カメラ）の4要素関係を説明すること
3. 新セクションは、「アクション！」による実行開始と、actへの出力（撮影）の概念を説明すること
...
1. SOUL.mdは、「4. 辞書アーキテクチャ」と「6. Phase 0: あるべき基盤」の間に、新しいセクション「5. シーン実行アーキテクチャ」を含むこと
...
**Objective:** 開発者として、アクター発話行がどのようにactへの出力に変換されるかを理解したい。これにより、撮影（出力イベント）の流れを把握できる。

#### Acceptance Criteria

1. 新セクションは、アクター発話が`act.アクター:talk()`によりactに記録されることを説明すること
2. 新セクションは、talkアクションのシーケンス（アクター選択、サーフェス切替、テキスト出力）を説明すること
3. アクター発話行が実行されるとき、ドキュメントはactへの出力フローを説明すること
