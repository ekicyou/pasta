...
- [x] 1. DSL パーサーの AST 型を `var_set_none` に対応させる
- [x] 1.1 `VarSet` 構造体の変数名フィールドを省略可能な型に変更する
...
- [x] 1.2 PEG 文法に `var_set_none` ルールを追加する
...
- [x] 1.3 パーサーに `var_set_none` の解析処理を追加する
...
- [x] 2. パーサー層の単体テストを追加する
- [x] 2.1 `var_set_none` のパーステストを追加する
...
- [x] 3. グローバル関数呼び出しの Lua コード生成を修正する
- [x] 3.1 (P) `＠＊XX()` の展開先を `GLOBAL.XX` に修正する
...
- [x] 3.2 (P) 生成 Lua コードのヘッダーに GLOBAL モジュール読み込みを追加する
  - `write_header()` に `local GLOBAL = require "pasta.global"` を `local PASTA` の次行として追加する
  - GLOBAL の使用有無に関わらず常時出力する
  - _Requirements: 1.6_

- [x] 4. `＄＝expr` 式文の Lua コード生成を実装する
- [x] 4.1 変数名なし（`name: None`）の場合に式文を出力する
...
- [x] 5. スナップショットテストを更新して全テストを通過させる
- [x] 5.1 GLOBAL ヘッダー追加によるスナップショット差分を承認する
...
- [x] 5.3 `＄＝expr` 式文のスナップショットテストを追加する
  - `＄＝＠fn()` → `SCENE.fn(act)` の式文出力を確認するスナップショットテストを追加する
  - `＄＝＠＊fn()` → `GLOBAL.fn(act)` の式文出力テストを追加する
  - _Requirements: 2.4, 2.5_

- [x] 6. LSP セマンティックトークンを `var_set_none` に対応させる
- [x] 6.1 変数名なしのトークン化処理を追加する
