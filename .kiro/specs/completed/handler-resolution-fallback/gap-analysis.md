...
**設計判断ポイント（確定）**: ローカルスコープを先に解決してからグローバルスコープを解決する順序で統一する。`scene.XX` → ローカル辞書 → `act.XX` → `GLOBAL.XX` → グローバル辞書 の順序。`act.XX`（`transfer_req_to_var` 等のランタイム内部メソッド）はゴースト開発者が `GLOBAL` で上書きする意味がないため、グローバルスコープより前の保護位置に配置する。ローカル・グローバルの括りを明確に分離し、各括り内では完全一致 → 前方一致の順に解決する。
