...
- yield後に同じアクターから継続しても、スポットタグ（`\p[0]`等）が再出力されるのは意図通り
...
1. **会話開始時の初期化**:
   ...
   - 現状: 最初の `talk()` 呼び出しでスポットタグ（`\p[0]`, `\p[1]`）は出力されるが、初期サーフェスは未指定
...
| スポットタグ | `\p[ID番号]` でスポット切り替えを指示（例: `\p[0]`, `\p[1]`, `\p[2]`） |
