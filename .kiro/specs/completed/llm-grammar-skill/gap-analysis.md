...
- **スコープ**: ゴースト開発者の辞書制作をサポートするVS Code Copilot Skill（SKILL.md形式）の新規作成。開発者が自然言語で指示し、LLMがPasta DSLコードに変換する作業を支援する
...
- **主要課題**: 情報の「集約と最適化」。LLMコンテキストウィンドウに収まるサイズで、自然言語→Pasta DSL変換に十分な情報密度を実現する必要がある
- **推定労力**: S（1〜3日）/ リスク: Low
