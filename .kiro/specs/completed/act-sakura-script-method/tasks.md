...
- [x] 1. (P) トランスパイラでアクター紐付きコード生成に修正する
...
- [x] 2. actランタイムに `sakura_script` メソッドを追加する
- [x] 2.1 (P) プロキシレイヤーに `sakura_script` メソッドを追加する
...
- [x] 2.2 (P) ACT_IMPL に `sakura_script` メソッドを追加し、グループ化ロジックを拡張する
...
- [x] 3. (P) sakura_builder でさくらスクリプトトークンを処理できるようにする
...
- [x] 4. テストを追加して全件パスを確認する
- [x] 4.1 (P) スナップショットテストでトランスパイル出力を検証する
...
- [x] 4.2 (P) `act_grouping_test.lua` に `sakura_script` グループ化テストを追加する
...
- [x] 4.3 (P) `sakura_builder_test.lua` に `sakura_script` ビルドテストを追加する
