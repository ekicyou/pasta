...
1. **アクター処理の詳細仕様【決定済み】**
   - ✅ **決定**: アクターは出現順に処理するが、file_attr属性の影響は受けない
   - ✅ **根拠**: アクターはシーンと異なり、ファイルレベル属性を継承しない設計
   - ✅ **処理**: FileItem::ActorScopeが出現したら、その時点でアクター処理を実行
   - ✅ **スコープ**: アクター内の単語定義はアクタースコープで処理される
   - **実装**: pasta_runeの参照実装を確認して対応

2. **ファイル属性のシャドーイング処理【明確化済み】**

1. **ファイル属性のシャドーイング処理【明確化済み】**
   - ✅ **確認済み**: file_attrはシャドーイング（上書き）により順序依存
   - ✅ **挙動**: 直後のグローバルシーンが参照、同一キーの再出現で上書き、シーン出現時の状態を継承
   - ✅ **影響**: HashMap列挙は使用不可（順序喪失により正しい継承が不可能）
   - ✅ **例**:
     ```pasta
     &author:A
     ＊シーン1  ← author=A を継承
     ...
     ＊シーン2  ← author=B を継承
     ```
   - **アクション**: pasta_rune Transpiler2 のコード生成部を参考に実装

2. **2パス vs 1パス【決定済み】**
   - ✅ **決定**: pasta_luaは1パス処理で完結
   - ✅ **根拠**: Lua言語とRune言語の設計差異により、Luaでは2段階処理が不要
   - ✅ **API**: `transpile_file()` を採用（pasta_runeのpass1相当だが、内部は1パス完結）
   - ✅ **実装**: pass2は実装しない
