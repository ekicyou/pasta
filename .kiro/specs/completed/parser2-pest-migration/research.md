...
// マーカー定義
sub    = _{ "-" | "－" }
dot    = _{ "．" | "." }  // 全角小数点と半角小数点の両方
```

- `sub?`: 符号（オプション、全角`－`または半角`-`）
- `digit+`: 整数部（必須、全角・半角数字）
- `(dot ~ digit+)?`: 小数部（オプション、**全角`．`または半角`.`**）

**重要**: `dot`は2種類の小数点を受け入れる
- 全角: `．` (U+FF0E FULLWIDTH FULL STOP)
- 半角: `.` (U+002E FULL STOP)
...
| 入力 | 正規化後 | 結果 | 観点 |
|------|---------|------|------|
| `123` | `123` | `Integer(123)` | 半角整数 |
| `１２３` | `123` | `Integer(123)` | 全角整数 |
| `-456` | `-456` | `Integer(-456)` | 半角マイナス |
| `－４５６` | `-456` | `Integer(-456)` | 全角マイナス |
| `3.14` | `3.14` | `Float(3.14)` | 半角小数点 |
| `３．１４` | `3.14` | `Float(3.14)` | **全角小数点** |
| `3．14` | `3.14` | `Float(3.14)` | 混在（数字半角+小数点全角） |
| `３.１４` | `3.14` | `Float(3.14)` | 混在（数字全角+小数点半角） |
| `－０．５` | `-0.5` | `Float(-0.5)` | 全角完全 |
| `-0.5` | `-0.5` | `Float(-0.5)` | 半角完全 |
| `9223372036854775807` | (same) | `Integer(i64::MAX)` | 境界値（最大） |
| `-9223372036854775808` | (same) | `Integer(i64::MIN)` | 境界値（最小） |
| `０` | `0` | `Integer(0)` | ゼロ（全角） |
| `0.0` | `0.0` | `Float(0.0)` | ゼロ（浮動小数点） |
