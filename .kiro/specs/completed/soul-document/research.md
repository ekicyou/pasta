...
**決定**: 本仕様のスコープに含め、Phase 0完了を目標とする

**理由**:
- Requirement 7 AC 4「Phase 0完了基準への貢献」を完全達成
- SOUL.md Section 5.6との整合性確保
- スコープを広げることで矛盾を解消

**対応方針**:
1. 失敗原因の特定（`shiori.load()`呼び出し失敗の詳細調査）
2. SHIORI.DLL初期化シーケンスの修正
3. 5テストすべてがパスすることを確認

**リスク**: 失敗原因が複雑な場合、調査時間が増加する可能性（軽減策: 段階的デバッグ）
