...
trにおける「トランスパイラー層」のみを実装対象とする。ランタイム層（Lua VM実行、coroutine管理、word/talk関数実装）はスコープ外。

**成果物**:
- Pasta AST → Lua コード生成ルールの仕様書（sample.lua のような形式を出力）
...
### Lua トランスパイラー化の目的
- Rune VM への依存性を排除（本仕様ではトランスパイラー層のみ対応）
- Pasta AST → Lua コード生成メカニズムの確立
- pasta_lua クレート内に言語非依存なトランスパイルロジックを実現

### Lua選択の理由（トランスパイラー層視点）
| 理由 | 詳細 |
|------|------|
| **シンプルな文法** | Lua のテーブル、関数、ローカル変数の生成が直線的 |
| **Unicode識別子サポート** | Lua 5.3以降は日本語シーン名・変数名をネイティブサポート |
| **メタテーブル拡張性** | ランタイム層での word/talk メソッド実装に有利（本仕様ではランタイム層外） |

### 権威的リファレンス
- `crates/pasta_core/src/parser/grammar.pest`: Pasta DSL文法定義
- `/SPECIFICATION.md`: Pasta DSL仕様書
- `grammar.md`: マーカー・制御フロー・IR定義
- **Reference Implementation**: `.kiro/specs/pasta-lua-specification/sample.lua` (手作業による期待出力)
- 既存 Rune トランスパイラー: `crates/pasta_rune/src/transpiler/code_generator.rs` (参考パターン)

## スコープ明確化

### ✅ 本仕様に含まれる（トランスパイラー層）
- Pasta AST ノードから Lua コード文字列への変換ルール（Requirements 0-1）
