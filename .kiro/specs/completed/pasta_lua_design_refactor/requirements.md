...
#### 目標1：子仕様の作成
本仕様の設計を元に、以下の子仕様を作成する：
1. **トランスパイラー仕様**: code_generator.rs の出力を本設計に合わせる
2. **Lua実装仕様**: 本設計に合致したLuaモジュール実装
3. **検索モジュール仕様**: Rust側のシーン辞書・単語辞書検索モジュールの仕様変更

#### 目標2：Luaスケルトンコード作成
以下のLuaファイルのスケルトンを作成する：
- `pasta/init.lua` - 公開API
- `pasta/act.lua` - アクションオブジェクト
- `pasta/scene.lua` - シーンレジストリ
- `pasta/actor.lua` - アクター定義
- その他必要と思われるLuaコード

**スケルトンの目的**: トランスパイラー出力結果をLuaビルドしてもエラーにならない程度のAPI定義を揃える。

- **要件フェーズ**: 設計原則と責務の明確化
- **設計フェーズ**: モジュール構造、API仕様、データフローの詳細設計
- **実装フェーズ**: 子仕様の作成 + Luaスケルトンコードの生成
...
5. The 設計ドキュメント shall varはAct側に移動したことを明記する（アクション期間中のみ有効な作業変数）
...
**設計原則**: スポット管理はシーン関数がactに与える「行動」の一種であり、「誰がシーンに参加するか、立ち位置はどこか」を指定する。スポット情報はActorProxyオブジェクトに保持される。

#### Acceptance Criteria
1. The 設計ドキュメント shall `act:clear_spot()` API仕様を記載する（全スポット割り当てリセット）
2. The 設計ドキュメント shall `act:set_spot(name, number)` API仕様を記載する（アクター位置指定）
3. The 設計ドキュメント shall シーン関数内での呼び出しパターン `act:set_spot("さくら", 0)` を例示する
4. The 設計ドキュメント shall スポット情報がActorProxyオブジェクトに保持される設計を明記する
...
### Requirement 10: 子仕様の作成
**Objective:** As a プロジェクト管理者, I want 本仕様から派生する子仕様が定義されている, so that 実装作業を分担できる

#### Acceptance Criteria
1. The 本仕様 shall トランスパイラー仕様（code_generator.rs修正）の子仕様を作成する
2. The 本仕様 shall Lua実装仕様（本設計に合致したLuaモジュール実装）の子仕様を作成する
3. The 本仕様 shall 検索モジュール仕様（Rust側シーン辞書・単語辞書）の子仕様を作成する
4. The 各子仕様 shall 本仕様の設計ドキュメントを参照する

### Requirement 11: Luaスケルトンコードの作成
**Objective:** As a トランスパイラ開発者, I want Luaスケルトンコードが存在する, so that トランスパイラー出力がLuaビルドでエラーにならない

#### Acceptance Criteria
1. The 本仕様 shall `pasta/init.lua` スケルトンを作成する（公開API定義）
2. The 本仕様 shall `pasta/act.lua` スケルトンを作成する（アクションオブジェクトAPI）
3. The 本仕様 shall `pasta/scene.lua` スケルトンを作成する（シーンレジストリAPI）
4. The 本仕様 shall `pasta/actor.lua` スケルトンを作成する（アクター定義API）
5. The 本仕様 shall `pasta/ctx.lua` スケルトンを作成する（環境コンテキストAPI）
6. The スケルトンコード shall トランスパイラー出力が参照するすべてのAPIを定義する
7. The スケルトンコード shall 実装は空でもよいが、シグネチャは正確に定義する
