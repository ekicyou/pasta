...
- [x] 1.1 (P) `generate_local_scene` 関数署名に `actors` パラメータを追加
...
- [x] 1.2 (P) `generate_local_scene` 内に set_spot ブロック生成ロジックを実装
...
- [x] 2.1 (P) `generate_global_scene` から `generate_local_scene` 呼び出しを修正
...
### コンパイル検証
- [x] Task 1.1 実行後: `cargo check -p pasta_lua` パス
- [x] Task 1.2 実行後: `cargo check -p pasta_lua` パス
- [x] Task 2.1 実行後: `cargo check -p pasta_lua` パス

### テスト検証
- [x] Task 3.1 ～ 3.4: 各テスト関数が `transpiler_integration_test.rs` に正しく追加される
- [x] Task 3.5: `cargo test --all` 完全パス（既存テスト＋新規テスト）

### 機能検証
- [x] 出力 Lua コード が要件 4 の「生成コード例」と一致
- [x] 複数行アクター宣言が 1 つの `__start__` で集約初期化される
- [x] アクター未定義時に `set_spot` が生成されない
