...
2. When ナビゲーションが必要なとき, the lua55-manual shall 「[← 前へ: N-1 – タイトル](前ファイル) | [目次](./README.md) | [次へ: N+1 – タイトル →](次ファイル)」形式のナビゲーションリンクを含む
3. When 最初の章（1章）の場合, the lua55-manual shall 「[← 目次](./README.md) | [N+1章 →](次ファイル)」形式を使用する
