...
- [ ] 1.1 (P) Rune VM での Template Literal 直接評価実現可能性を確認
...
- [ ] 3.1 (P) VariableManager を Rune Context に `ctx.local`/`ctx.global` として公開
...
- [ ] 4.1 ローカル変数代入（`$変数: 値`）の Rune コード生成
...
- [ ] 4.2 グローバル変数代入（`$*変数: 値`）の Rune コード生成
  - `Statement::VarAssign { name, scope: Global, value }` を受け取る
  - `ctx.global.name = value` 形式の Rune コード生成
  - 値は `Expr::Literal` として文字列リテラルに変換
  - _Requirements: 2.2, 2.3_

- [ ] 4.3 変数代入時の識別子バリデーション
  - 無効な識別子（空文字列、特殊文字など）の検出
  - 無効な場合 `PastaError::InvalidIdentifier` を生成しトランスパイル失敗
  - エラーには span 情報（ファイル位置）を含める
  - _Requirements: 2.4_

### 5. Transpiler 層：アクション行での変数参照（VarRef/Talk）の処理実装
- [ ] 5.1 ローカル変数参照（`$変数`）の Talk イベント生成
  - `src/transpiler/mod.rs` の `transpile_speech_part_to_writer()` 関数を修正
  - `SpeechPart::VarRef { name, scope: Local }` を受け取る
  - Phase 0 結果に基づいて以下いずれかを生成：
    - 直接評価可能: `yield Talk(\`${ctx.local.name}\`);`
    - 不可の場合: `let a = ctx.local.name; yield Talk(\`${a}\`);`
  - _Requirements: 1.1, 3.1_

- [ ] 5.2 グローバル変数参照（`$*変数`）の Talk イベント生成
  - `SpeechPart::VarRef { name, scope: Global }` を受け取る
  - 同様に Phase 0 結果に基づいて以下いずれかを生成：
    - 直接評価可能: `yield Talk(\`${ctx.global.name}\`);`
    - 不可の場合: `let a = ctx.global.name; yield Talk(\`${a}\`);`
  - _Requirements: 1.2, 3.1_

- [ ] 5.3 変数参照時の未定義チェックと エラーハンドリング
...
- [ ] 6.1 ローカル変数キー による word 検索
  - `src/transpiler/mod.rs` の `transpile_speech_part_to_writer()` 関数で FuncCall 処理を修正
  - `SpeechPart::FuncCall { name: "$変数", ... }` を検出
  - Phase 0 結果に基づいて以下いずれかを生成：
    - 直接評価可能: `yield Talk(pasta_stdlib::word(module, \`${ctx.local.変数名}\`, []));`
    - 不可の場合: `let a = ctx.local.変数名; yield Talk(pasta_stdlib::word(module, \`${a}\`, []));`
  - _Requirements: 1.1, 4.1_

- [ ] 6.2 グローバル変数キー による word 検索
  - `$*変数` パターンを検出
  - 同様に Phase 0 結果に基づいて以下いずれかを生成：
    - 直接評価可能: `yield Talk(pasta_stdlib::word(module, \`${ctx.global.変数名}\`, []));`
    - 不可の場合: `let a = ctx.global.変数名; yield Talk(pasta_stdlib::word(module, \`${a}\`, []));`
  - _Requirements: 1.2, 4.1_

- [ ] 6.3 検索キーが空の場合のエラーハンドリング
  - キー値が空文字列の場合、Runtime 例外または診断エラーを生成
  - エラーに span 情報を含める
  - _Requirements: 4.2_

### 7. Transpiler 層：動的シーン呼び出し（`>$変数`）の処理実装
- [ ] 7.1 ローカル変数ラベル によるシーン呼び出し
  - `src/transpiler/mod.rs` の `transpile_jump_target_to_search_key()` 関数を修正
  - `JumpTarget::Dynamic("$変数")` の場合、Phase 0 結果に基づいて以下いずれかを生成：
    - 直接評価可能: `for a in crate::pasta::call(ctx, \`${ctx.local.変数名}\`, #{}, []) { yield a; }`
    - 不可の場合: `let a = ctx.local.変数名; for b in crate::pasta::call(ctx, \`${a}\`, #{}, []) { yield b; }`
  - 旧式 `@var_name` 形式（`@` プレフィックス付与）の生成ロジックを削除
  - _Requirements: 1.1, 5.1_

- [ ] 7.2 グローバル変数ラベル によるシーン呼び出し
  - `$*変数` パターンを検出（例：`>$*next_scene`）
  - 同様に Phase 0 結果に基づいて以下いずれかを生成：
    - 直接評価可能: `for a in crate::pasta::call(ctx, \`${ctx.global.変数名}\`, #{}, []) { yield a; }`
    - 不可の場合: `let a = ctx.global.変数名; for b in crate::pasta::call(ctx, \`${a}\`, #{}, []) { yield b; }`
  - _Requirements: 1.2, 5.1_

- [ ] 7.3 ラベルキーが空の場合のエラーハンドリング
  - キー値が空文字列の場合、Runtime 例外または診断エラーを生成
  - エラーに span 情報を含める
  - _Requirements: 5.2_
...
- [ ] 8.2 既存テストの修正
  ...
  - L31, L37 の `ctx.var` 記載を `ctx.local`/`ctx.global` に更新（ドキュメント的変更のみ）
  - 実装ロジックへの影響なし
  - _Requirements: 6.2_

### 9. ユニットテスト：各コンポーネント単独検証 (P)
- [ ] 9.1 (P) VariableManager の Local/Global スコープ検証
...
- [ ] 9.2 (P) Transpiler の VarAssign 処理検証
...
### 11. エラーハンドリング・診断テスト
- [ ] 11.1 (P) 無効な変数名でのエラー検出テスト
  - 空の変数名、特殊文字含む名前などを使用
  - `PastaError::InvalidIdentifier` が生成されることを確認
  - エラーメッセージに span 情報（行・列）が含まれることを確認
  - _Requirements: 2.4, 6.2_

- [ ] 11.2 (P) 未定義変数参照でのエラー検出テスト（オプション）
  - 定義されていない変数を参照
  - Runtime エラーまたは診断エラーが生成されることを確認（実装段階で決定）
  - _Requirements: 3.2_

- [ ] 11.3 (P) 空の検索キー/ラベルキーでのエラー検出テスト
  - 空の値を持つ変数で word 検索またはシーン呼び出しを実施
  - Runtime 例外または診断エラーが生成されることを確認
