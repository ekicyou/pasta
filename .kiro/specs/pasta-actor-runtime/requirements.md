...
> **assumption（マーカー粒度）**: presentation event の具体的なマーカー種別・粒度・データ表現（列挙型／スキーマ）は、現状の Lua 側トークン（`{type="talk", actor, text}` 等）と既存さくらスクリプト出力からバイト不変で逆算できる最小集合とする。確定はディスカッション／設計フェーズに委ねる（OPEN QUESTION Q1）。
