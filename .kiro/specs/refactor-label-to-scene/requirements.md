...
本リファクタリングの置換原則により、実質的に**置換対象外となる箇所は以下の通りです**：

### 置換対象外の理由

| 項目 | 理由 |
|------|------|
| **文法マーカー文字** (`＊`、`・`) | 「ラベル」という単語ではなく、DSL構文として定義された記号 |
| **パッケージ名** (`pasta`) | 「ラベル」を含まない |
| **Git履歴（過去のコミットメッセージ）** | 変更対象外（新規に追加される内容は置換対象） |
| **外部ライブラリAPI名** | 現在「label」を含むAPI該当なし |
