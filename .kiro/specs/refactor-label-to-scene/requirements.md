...
**基本原則**: プロジェクト内の全ての「ラベル」（日本語）、「label」（英語）を機械的に「シーン」、「scene」に置換します。

**複数形の機械的置換**: 英語の複数形 `labels` も `scenes` に置換します。

**Rustモジュール名の特殊ルール**: 
- **例外的に単数形化**: `labels.rs` → `scene.rs`（`scenes.rs` ではない）
- **理由**: Rust規約では複数形モジュール名よりも単数形が推奨される（例: `error.rs`、`engine.rs`）
- **議題1で決定**: この機会に複数形を単数形に正規化する
- **適用範囲**: ファイル名のみ。コード内の変数・フィールド名 `labels` は機械的に `scenes` に置換
...
**例外**: Out of Scopeセクションを参照。理由が明示されている箇所のみ置換しない。
...
- **文法マーカー文字**: `＊`（グローバルシーン）、`・`（ローカルシーン）のマーカー文字自体
  - **理由**: DSL構文として定義済み、既存スクリプト互換性を保つ
  
- **Cargo.tomlのパッケージ名**: `pasta`
  - **理由**: crates.io公開済み、変更には別プロジェクトが必要
...
- **完了済み仕様ドキュメント**: `.kiro/specs/completed/**/*.md` も全て置換対象
  - **理由**: 用語統一を優先、歴史的記録はgit履歴で保持
  
- **現在の仕様ディレクトリ名**: `.kiro/specs/refactor-label-to-scene/` 自体も最後に `refactor-scene/` に変更
  - **理由**: 一貫性のため（Req 8.3）

### 機能変更なし

- **動作・セマンティクス**: APIの振る舞いや実行結果に変更なし
- **文法仕様**: DSL構文ルールに変更なし
