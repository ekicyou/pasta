...
- [ ] 1.3 (P) parsers/req.rs の `#[cfg(test)]` 化
...
- [ ] 1.4 (P) hglobal モジュールの `#[allow(dead_code)]` 精査
...
- [ ] 1.5 (P) lua_request.rs の `#[allow(dead_code)]` 精査
...
- [ ] 2. unsafeブロックのSAFETYコメント付与
- [ ] 2.1 shiori.rs の unsafe impl Send/Sync にSAFETYコメント追加
...
- [ ] 2.2 (P) hglobal/mod.rs の unsafe ブロックにSAFETYコメント追加
...
- [ ] 2.3 (P) hglobal/windows_api.rs の unsafe ブロックにSAFETYコメント追加
...
- [ ] 2.4 (P) windows.rs の extern "C" 関数にSAFETYドキュメント追加
...
- [ ] 3. FFI境界の入力検証強化
- [ ] 3.1 windows.rs の load 関数にNULLチェック追加
...
- [ ] 3.2 (P) windows.rs の request 関数にNULLチェック追加
...
- [ ] 4. プロダクションコードのパニック除去
- [ ] 4.1 lua_request.rs の parse_key_value 内 unwrap()/panic!() 変換
...
- [ ] 4.2 (P) parsers/req.rs の unwrap()/panic!() 変換
...
- [ ] 5.2 (P) lua_request.rs と req.rs のパースロジック重複確認
