...
- [ ] 1. `KeyWords` ASTを複数キー対応に拡張し、既存参照箇所をすべて修正する
...
- [ ] 2. PEG文法に複数キー構文を追加し、パーサーを更新する
- [ ] 2.1 PEG文法に `key_list` ルールを追加し `key_words` ルールを更新する
...
- [ ] 2.2 `parse_key_words()` を `key_list` 内の全 `id` を `names` に収集するよう更新し、テストを追加する
...
- [ ] 3. (P) pasta_lua のレジストリ登録を全キー対応にする
