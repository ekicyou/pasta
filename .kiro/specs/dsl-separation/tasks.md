...
- [ ] 1. pasta_dsl クレート基盤を構築する
- [ ] 1.1 pasta_dsl ディレクトリ構造とCargo.tomlを作成する
...
- [ ] 1.3 parser モジュールを pasta_core から移動する
...
- [ ] 1.4 ParseError 型を pasta_core から移動する
  - `src/error.rs` を新規作成
...
- [ ] 1.5 テストファイルを pasta_core から移動する
  - `tests/` ディレクトリを作成
...
- [ ] 1.6 pasta_dsl の README.md を作成する
  - `README.md` を新規作成
...
- [ ] 2. pasta_core から parser 関連を完全除去する
- [ ] 2.1 pasta_core の lib.rs から parser を除去する
  - `pub mod parser;` 行を削除
  - `pub use parser::*;` を削除
...
- [ ] 2.2 pasta_core の error.rs から ParseError を除去する
...
- [ ] 2.3 pasta_core から parser ディレクトリとテストファイルを削除する
...
- [ ] 3. pasta_lua を pasta_dsl に直接依存させる
- [ ] 3.1 pasta_lua の Cargo.toml に pasta_dsl 依存を追加する
  - `[dependencies]` セクションに `pasta_dsl.workspace = true` を追加
  - pasta_core への依存を維持（registry 層のため）
  - 依存順序を確認（pasta_dsl, pasta_core, mlua 等）
  - _Requirements: 2.3_

- [ ] 3.2 pasta_lua の import パスを pasta_dsl に変更する
...
- [ ] 4. ワークスペース全体で pasta_dsl を統合する
- [ ] 4.1 Cargo.toml のワークスペース依存に pasta_dsl を追加する
...
- [ ] 全6要件が実装されている
- [ ] 8ドキュメントファイルが更新されている
- [ ] pasta_dsl が独立して利用可能である（`cargo build -p pasta_dsl` 成功）
- [ ] pasta_core から parser が完全に除去されている
- [ ] pasta_lua が pasta_dsl に直接依存している
