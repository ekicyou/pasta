...
- [x] 2.1 (P) ピクトグラム画像生成ロジック
  - ImageGenerator モジュール実装（256x512px 透過PNG生成）
  - トイレマーク風人型アイコン描画（頭部: 円、胴体: 台形、手足: 線）
  - 表情バリエーション実装（9種: 笑顔 `^ ^`, 通常 `-  -`, 照れ `> <`, 驚き `o o`, 泣き `; ;`, 困惑 `@ @`, キラキラ `* *`, 眠い `= =`, 怒り `# #`）
  - キャラクター色分け（sakura: ライトブルー #4A90D9, kero: ライトグリーン #4AD98A）
...
全9要件、27個のAcceptance Criteriaをカバー。
