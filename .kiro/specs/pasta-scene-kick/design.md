...
7. ✓ **`kick.drop`（teardown 競合破棄）の UX** → 解決済み（ディスカッション #3）。**ベストエフォート**: ack は sink 呼び出し成立で返す（実 mailbox 到達は保証しない）。`MAILBOX.load_full()=None`（reload/teardown 中）は黙って破棄＋`tracing`（`seam="kick.drop"`）。確実な到達確認 ack 往復経路は新設しない（依存方向・非同期 ack の複雑化を回避・reload は一瞬で再キックで足りる）。
