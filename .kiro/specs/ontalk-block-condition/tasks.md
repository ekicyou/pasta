...
- [x] 1. ブロック判定ロジックを仮想ディスパッチャに実装する
- [x] 1.1 BLOCKED_STATUSES テーブルと M.is_blocked() 公開関数を追加する
...
- [x] 1.2 dispatch() の先頭に M.is_blocked() ブロックガードを追加する
  ...
  - _Requirements: 1, 2, 5_
- [x] 1.3 (P) pasta_scripts 版の check_hour() と check_talk() から talking/choosing 個別チェックを削除する
  ...
  - _Requirements: 2_
- [x] 1.4 (P) hello-pasta サンプルゴーストの virtual_dispatcher コピーに同じ変更を適用する
...
- [x] 2. テストスイートを更新する
- [x] 2.1 (P) 旧 talking/choosing ブロック検証テストを dispatch() 経由に書き直す
  ...
  - _Requirements: 3_
- [x] 2.2 (P) dispatch() 経由のブロック条件テスト18件を Lua spec に追加する
