...
- [ ] 2. パーサー層のリファクタリング
- [ ] 2.1 既存 Span 呼び出し箇所の修正（pasta_core）
...
- [ ] 3. Action enum のリファクタリング
- [ ] 3.1 Action enum を named fields 化
...
- [ ] 3.2 パーサー層での Action 生成修正（pasta_core）
...
- [ ] 4. トランスパイラ層のリファクタリング（pasta_rune）
- [ ] 4.1 pasta_rune トランスパイラの Action パターンマッチ修正
...
- [ ] 4.2 pasta_rune テストの Action 生成修正
...
- [ ] 5. トランスパイラ層のリファクタリング（pasta_lua）
- [ ] 5.1 pasta_lua トランスパイラの Action パターンマッチ修正
  - `CodeGenerator::generate_action()` で Action パターンマッチを named fields 形式に変更（推定 6 箇所）
  - `match action { Action::Talk { text, span } => ... }` 形式に統一
  - span フィールドからソース位置情報を取得可能に
  - _Requirements: 3.2, 7_

- [ ] 5.2 pasta_lua テストの Action 生成修正
  - テストコード内の Action 生成箇所（推定 10+ 箇所）を named fields 形式に変更
  - テスト用 Span は `Span::default()` または `Span::new(1, 1, 1, 1, 0, 0)` を使用
  - _Requirements: 3.2, 6_

- [ ] 6. テストカバレッジの拡充
- [ ] 6.1 (P) Span 構造体のユニットテスト
