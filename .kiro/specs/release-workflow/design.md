...
- **完遂保証（no half-done）**: 全ターゲット（crates.io 全クレート・Marketplace・タグ・Release）成功までリリースを「完了」としない。失敗しやすい手順（特に Marketplace）はセッション内バックオフ → ScheduleWakeup 永続リトライで完遂まで粘る（Req 11）
...
- **無人（セッション閉・マシンオフ）でのリリース自動完遂**: 第2段リトライは ScheduleWakeup の同一セッション継続で実現するため、完遂までセッションを開いておく前提。セッションを完遂前に閉じた場合は停止し、手動再実行（Resume Mode）で続行する。cron / Desktop Scheduled Tasks / 別セッション起動は使わない
...
- Runtime: ScheduleWakeup（`/loop` 動的ペーシング）— 第2段の同一セッション内待機→再開による永続リトライ（Req 11.2–11.4）。別基盤・別セッションは不要 (P1)
- Infra: ハーネスのプッシュ通知機構 — 待機ループ中の定期エスカレーション通知・非一時障害の即時通知（Req 11.8）(P1)
...
- Stage C の 2 トラックは並行。Track Y（隔離・完遂必須）の失敗は他トラックを妨げないが、未公開のまま完了せず Req 11 の ScheduleWakeup 再試行で完遂する（Req 8.4）。
...
### 共通リトライ戦略（二段: 短期バックオフ → ScheduleWakeup 永続リトライ）

外部サービス通信（`cargo publish`, `vsce publish`, `gh pr merge`, `git push`, `gh release create`）は相手側ビジー・レート制限・一時的ネットワーク障害で失敗し得る。**完遂保証（no half-done）**のため、Req 11 に基づき**同一セッション内**で二段に粘る（議題: ScheduleWakeup のみ採用。cron / Desktop タスク / 別セッション起動は使わない）:

```
第1段（短期バックオフ）: 1分 → 2分 → ... → 10分（初回+10回=11回、累計約55分）
        短い一時障害の即時吸収。待機は ScheduleWakeup を基本とし、
        ごく短い待機（〜1分）のみ Start-Sleep 可。前景の長時間 sleep は使わない。
第2段（ScheduleWakeup 永続リトライ）: 第1段で未完了が残れば、ScheduleWakeup で
        次回時刻（既定 30〜60 分間隔）まで待機 → 同一セッションが再開し未完了分のみ続行。
        全完遂まで繰り返す。回数・累計時間に固定上限を設けない（Req 11.2–11.4, 11.7）。
        ※ ScheduleWakeup は同一セッション継続のため多重実行は起きない。
```

**セッション維持が前提**: 第2段は ScheduleWakeup（同一セッションの待機→再開）であり、**セッションを開いている限り**完遂まで継続する。セッションを完遂前に閉じた場合は停止し、**手動で `/kiro-impl` を再実行**すれば Resume Mode（Req 9.5）で未完了分から続行する（unattended・マシンオフでの完遂は Non-Goal）。

**一時障害 vs 非一時障害の判別（Req 11.6）**: ビジー/レート制限/タイムアウト/5xx 等は**一時障害**として第2段（ScheduleWakeup 再試行）へ。認証無効・権限不足・ビルドエラー・コンフリクト等は**非一時障害**として再試行せず未完了報告し、開発者対応後に Resume で完遂する。
...
flowchart TD
    ...
    Y --> Y1[隔離 完了とせず 第2段 ScheduleWakeup 再試行 で完遂]
    D --> D1[第2段 ScheduleWakeup 再試行 で完遂まで]
```
...
| 11.2–11.4   | 短期バックオフ→ScheduleWakeup 永続リトライ（同一セッション待機→再開・冪等） | 共通リトライ / 完遂保証節 | 共通リトライ戦略 |
...
| 11.8        | 待機ループ中の定期エスカレーション通知・セッション内試行履歴 | 完遂保証節 | 共通リトライ戦略・完遂保証 |
...
| Track Y: VsixPublish   | C     | Marketplace 公開（隔離・完遂必須）       | 4.3–4.7, 11.1–11.8 | vsce (R3), ScheduleWakeup                         | 隔離/必須 |
...
**Responsibilities & Constraints**: Track X と並行実行可能（他トラックを**ブロックしない＝隔離**、Req 8.4）。ただし Marketplace 公開は**完遂必須**であり、未公開のまま完了しない。`vsce publish` 失敗時は第1段バックオフ → なお失敗なら第2段 ScheduleWakeup 再試行で完遂まで粘る（Req 11）。Marketplace に既に当該バージョンが存在すれば公開をスキップ（冪等）。成功時は Marketplace URL を記録 (4.7)。非一時障害（`VSCE_PAT` 無効等）はスケジュール再試行に載せず未完了報告する（11.6）。
...
各ステージはゲート方式で制御される。Stage A の失敗はローカルのみで停止（対外影響なし）。**Stage B（統合）の失敗は不可逆な Stage C/D をブロックする**（安全ゲート）。Stage C Track X の失敗は Stage D をブロックするが main 統合は保持される。Track Y の失敗は他トラックから隔離される。外部通信の一時障害はいずれも**第1段バックオフ → 第2段 ScheduleWakeup 再試行**で完遂まで粘り、**未完了のまま「完了」としない**（Req 11、no half-done）。非一時障害はスケジュール再試行に載せず未完了報告する。
...
| C / Track X         | cargo publish 失敗     | 第1段 → 第2段 ScheduleWakeup 再試行（未公開分のみ、Stage D は全公開後） | **main 統合は保持**・既公開クレートは残す |
| C / Track Y         | vsce publish 失敗      | 第1段 → 第2段 ScheduleWakeup 再試行（一時障害）/ 未完了報告（非一時）。**隔離するが完了とせず** | 不要（他トラック非ブロック・Req 11 で完遂） |
| D / Phase 7         | タグ push / gh release 失敗 | 第1段 → 第2段 ScheduleWakeup 再試行（完遂まで）        | 完了とせず Req 11 で完遂           |
...
| ローカルビルド成果物（ghost dll.zip / nar / VSIX） | 生成 | **手動 resume（新セッション）時のみ再生成**（新ワークツリーに成果物が無いため Release 添付用に再ビルド）。ScheduleWakeup の同一セッション再開時は成果物が保持されており再ビルド不要 |
...
- 公開可否判定は crates.io / Marketplace / GitHub Release / タグの**実状態**を都度確認して行う（タスク状態に依存しない）。これにより Req 9.3（前回実行に依存しない独立動作）と両立する。具体的な状態確認手段:
  - **crates.io**: `cargo publish` のドライ実行は不可のため、各クレートの index を HTTP 確認（`https://crates.io/api/v1/crates/<crate>/<version>`）。存在すれば公開済みとしてスキップ。`cargo publish` が「already exists」で失敗した場合も成功扱い。
  - **Marketplace**: `vsce show <publisher>.<extension> --json` の versions に当該バージョンがあれば公開済み。
  - **GitHub Release**: `gh release view vV`（存在で作成済み、アセット一覧で添付不足を判定）。
  - **タグ**: `git ls-remote --tags origin vV`（リモート push 済み判定）。
- 完全公開（全クレート公開・タグ push・Release 作成）に到達した時点で V のリリースは完了とみなし、次回実行は通常の新規リリース（V の PATCH+1 提案）となる。

### 完遂保証と ScheduleWakeup 永続リトライ（Req 11）

**目的**: 相手側サーバーのビジー等で失敗しやすい手順を、**同一セッション内**で完遂まで粘る。**中途半端な完了を起こさない**。

**機構（議題決定: ScheduleWakeup のみ）**: 第2段は **ScheduleWakeup**（`/loop` 動的ペーシング）で実現する。第1段（短期バックオフ約55分）で未完了ターゲットが残れば、ScheduleWakeup で次回時刻（既定 30〜60 分間隔）まで待機し、**同一セッションが再開**して未完了分を続行する。全ターゲット完遂まで繰り返す。

- **同一セッション継続**: 別セッションを起動しないため**多重実行が原理的に起きない**（single-flight ロック不要）。cron / Desktop Scheduled Tasks / 別セッション起動は**使わない**。
- **環境**: 現セッション（ローカル Windows・全ツールチェーン・env 認証）をそのまま使うため、ヘッドレス認証や Windows ビルドの実行基盤問題は生じない。
- **再開時の動作**: 各再開で**未完了分のみ**を冪等に続行する（公開可否は外部実状態で都度判定）。Resume Mode の冪等判定をそのまま用いる。
- **完遂で終了**: 全ターゲット完遂で待機ループを終了し「完了」を報告する。

**セッション維持が前提（Non-Goal の明示）**: 本機構は**セッションを開いている限り**完遂まで継続する。セッションを完遂前に閉じた／マシンを落とした場合は停止する。その場合は**手動で `/kiro-impl release-workflow` を再実行**すれば Resume Mode（Req 9.5）が外部実状態から未完了分を検知して続行する。**無人（セッション閉・マシンオフ）での自動完遂は Non-Goal** とする。

**エスカレーションと可観測性（Req 11.8）**:
- 長時間の待機ループでも**沈黙させない**。試行履歴（累計試行回数・初回/最終試行時刻・最終エラー＋分類・各ターゲット状態）をセッション内で保持する。
- **定期通知**: 既定で「**5 回ごと または 24 時間経過ごと**」に開発者へプッシュ通知（「リリース vX.Y.Z 未完遂・継続再試行中／累計 N 回／最終エラー: … ／分類: 一時」）。これにより「完遂待ち」と「一時障害と誤分類した実質詰み」を判別でき、開発者が手動中止・再分類できる。
- リトライは通知後も継続する（リトライ論理に上限は設けない。Req 11.7）。ただし自律継続の実寿命は**同一セッション／ScheduleWakeup ループの TTL（約7日）**に律速される。これを超える長期障害では、定期通知を見た開発者が手動で `/kiro-impl` を再実行すれば Resume Mode が続行する（無人完遂が Non-Goal であることと整合）。

**非一時障害の扱い（Req 11.6）**: 認証無効・権限不足・ビルドエラー・マージコンフリクト等、リトライで解消しない種別は ScheduleWakeup 待機に載せず、「未完了・要対応」として原因と必要対応を**即時通知**し、開発者対応後に Resume Mode で完遂する。

**完了判定（Req 11.1, 11.5）**: 全ターゲット（crates.io 全クレート・Marketplace・タグ push・GitHub Release）成功で「完了」。未完了が残る限り「未完了（再試行待ち）」として、残作業・障害分類・次回 ScheduleWakeup 予定を報告する。
