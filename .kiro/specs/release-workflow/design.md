...
- Infra: ハーネスのプッシュ通知機構 — 第2段の定期エスカレーション通知・非一時障害の即時通知（Req 11.8）(P1)
...
| `release/.release-retry-status-vX.Y.Z.json`         | 第2段再試行の試行履歴（**gitignore 対象・機械可読**・完遂で削除）              | C/D (Req 11.8) |
...
| 11.8        | 第2段の定期エスカレーション通知・試行履歴記録 | 完遂保証節 / status レコード | 共通リトライ戦略・完遂保証 |
...
**エスカレーションと可観測性（議題2 決定・Req 11.8）**:
- 第2段は上限なく自律継続するが、**沈黙させない**。試行履歴を機械可読の status レコードに保持する: `{version, 各ターゲット状態, 累計試行回数, 初回/最終試行時刻, 最終エラー＋分類}`。新ワークツリー／cron 起動を跨いで参照できるよう、リポジトリ作業領域配下の **gitignore 対象ローカル status ファイル**（例: `release/.release-retry-status-vX.Y.Z.json`）に記録し、cron 起動ごとに更新、完遂時に削除する（無ければ初回として初期化）。
- **定期通知**: 既定で「**5 回ごと または 24 時間経過ごと**」に開発者へプッシュ通知（「リリース vX.Y.Z は未完遂・継続再試行中／累計 N 回／最終エラー: … ／分類: 一時」）。これにより「完遂待ち」と「一時障害と誤分類した実質詰み」を判別でき、必要なら開発者が手動中止・再分類できる。
- リトライ自体は通知後も継続する（完全自律・時間無制限と整合。Req 11.7）。

**非一時障害の扱い（Req 11.6）**: 認証無効・権限不足・ビルドエラー・マージコンフリクト等、リトライで解消しない種別はスケジュール再試行に載せず、「未完了・要対応」として原因と必要対応を**即時通知**し、開発者対応後に通常の再実行が Resume Mode で完遂する。
