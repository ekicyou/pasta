...
### 12. pasta-grammar-specification
**カテゴリ**: 文法/ドキュメント  
**完了日**: 2025-12-20  
**概要**: 権威仕様に合わせた文法整合と破壊的変更反映（Jump廃止、Sakura半角限定、text_part修正）。GRAMMAR.md を全面改訂し、pest/AST/Transpiler/テストを整合させて全テスト緑を維持。

**主要成果物**:
- GRAMMAR.md の仕様準拠リライト（Sync/Event削除、Callのみ、Sakura半角化、例示更新）
- Jump ルール・AST・Transpiler分岐の削除とテスト更新
- Sakura エスケープ/括弧の半角限定、`text_part` の `$` 除外修正

---

## 統計

- **「完了」扱い仕様**: 12件（⚠️ 実装品質不十分、再評価必要）
- **実施期間**: 2025-11-27 ～ 2025-12-20（推定）
- **カテゴリ分布**:
  - トランスパイラ: 3件
  - アーキテクチャ: 1件
  - ランタイム: 1件
...
#### 「完了」仕様の実態
これら12件の多くは `areka-P0-script-engine` の**小規模改修パッチ**で、根本的な設計問題は手つかず（例外: pasta-grammar-specification は文法整合の破壊的変更を反映）。
