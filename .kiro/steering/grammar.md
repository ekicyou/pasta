...
- **プロパティ変数**: `＄％prop.path` - SSP共有プロパティの読み書き（SET/GET/インラインGET）

### 制御フロー
- **Call文** (`＞label`): サブルーチン呼び出し、実行後復帰
- **特殊Call**: `＞ゴースト終了（ミリ秒）` / `＞チェイントーク`（`＞yield`エイリアス）
- **Luaブロック**: 複雑なロジックをLua言語で記述

### シャッフル＆順次消費方式
同名シーン・単語の選択は**シャッフル＆順次消費**で行われる（純粋ランダムではない）：
1. 全候補をシャッフルし先頭から順に返却する
2. 全候補を消費したら再シャッフルして繰り返す
3. **サイクル内保証**: 1サイクル内で同じ値は再選択されない
4. 重複シーン（`＊OnTalk` 複数定義）・単語参照の両方に適用される

### インライン要素空白ルール
`＠`・`＄`の識別子は**最長一致**で解析される。日本語文字（平仮名・カタカナ・漢字）はXID_CONTINUEに含まれるため、後続の日本語テキストが識別子に吸収される。
- ✅ `＠地名　から` — 空白がトークン区切り（空白は出力されない）
- ❌ `＠地名から` — 「地名から」が1識別子として吸収される  
- `＠＠` — リテラルの「＠」を出力するエスケープ

### 複数キー単語定義
`＠キー1、キー2：値1、値2` — 同一値リストを複数のキー名に同時登録。  
グローバル・ローカル・アクタースコープ全てで有効。  
例: `＠女性、水の妖精：水無灯里、アリス・キャロル`
