...
- `＊時報12` `＊時報その他` は時報（毎正時のあいさつ）用のシーンである。`＄時１２` には現在時刻（12 時間表記）が自動的に入る。ほかに使える日時変数は[日時変数](../grammar/variables.md#日時変数)を参照。特定の時刻専用のシーンがなければ「その他」がフォールバックとして使われる。
...
`OnMouseDoubleClick` はゴーストがダブルクリックされたときのイベントである。ステップ 3・4 と同じく、同名シーンを複数並べてランダムに選ばせている。書き方の文法はこれまでと変わらない。ダブルクリックされた部位などイベントの付加情報（Reference）を会話に使いたいときは、[エンジンが値を入れる変数](../grammar/variables.md#リクエスト変数reference)を参照。
