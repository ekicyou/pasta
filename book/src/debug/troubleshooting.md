...
ただし、**デバッガ接続以前にゴーストがそもそも起動しない・まったく喋らない**場合は、本ページではなく [起動シーケンスとモジュール解決](../reference/startup.md) を先に参照する。起動シーケンスのどこかでロードに失敗している可能性が高く、その場合は 500 応答の `X-ERROR-REASON` ヘッダとログファイルの `fatal=true` の行に原因が出ている。切り分けの出発点が本ページとは異なるため、先にそちらで起動の成否を確定させること。
