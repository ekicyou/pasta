...
**セマンティクス**: 同一の値リストを複数のキー名に同時登録する。

- コロン（`：`/`:`）の左側にカンマ区切りで1つ以上のキーを指定できる
- キー区切りは全角読点（`、`）・全角コンマ（`，`）・半角カンマ（`,`）のいずれも使用可
- キーが1つの場合は従来形式と同一（後方互換）
- グローバル・ローカル・アクタースコープ（`actor_scope_item`）すべてで有効
- エラーハンドリング: コロン省略・空キーは PEG 文法 `kv_marker` / `id` ルールによりパースエラー
