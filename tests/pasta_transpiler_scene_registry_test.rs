use pasta::transpiler::SceneRegistry;
...
fn test_label_registry_with_local_scenes() {
    ...
    // Verify parent
    let parent = registry.get_scene(parent_id).unwrap();
    ...
    let local2 = registry.get_scene(local2_id).unwrap();
...
fn test_label_registry_duplicate_names() {
    ...
    let label1 = registry.get_scene(id1).unwrap();
    let label2 = registry.get_scene(id2).unwrap();
    let label3 = registry.get_scene(id3).unwrap();
