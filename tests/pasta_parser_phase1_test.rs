...
"#;
    ...
    assert!(
        result.is_ok(),
        "Half-width backslash should be accepted: {:?}",
        result.err()
    );
}
    ...
    if let Ok(file) = &result {
        ...
        if let Some(pasta::parser::Statement::Speech { content, .. }) = speech {
            let has_sakura = content
                .iter()
                .any(|p| matches!(p, pasta::parser::SpeechPart::SakuraScript(_)));

            if !has_sakura {
                eprintln!(
                    "SUCCESS: Full-width backslash is NOT recognized as Sakura (Phase 1 complete)"
                );
            } else {
            ...
            assert!(
                !has_sakura,
                "Full-width backslash should NOT be recognized as Sakura script"
            );
        }
    }
}
...
"#;
    ...
    assert!(
        result.is_ok(),
        "Half-width brackets should be accepted: {:?}",
        result.err()
    );
}
...
";
    ...
    if result.is_ok() {
        if let Ok(file) = &result {
            ...
            if let Some(pasta::parser::Statement::Speech { content, .. }) = speech {
                let sakura_parts: Vec<_> = content
                    .iter()
                ...
                if sakura_parts.is_empty() {
                    eprintln!(
                        "SUCCESS: Full-width Sakura syntax NOT recognized (Phase 1 complete)"
                    );
                } else {
        ...
        eprintln!(
            "NOTE: Parse failed (expected if full-width causes issues): {:?}",
            result.err()
        );
    }
...
"#;
    ...
    assert!(
        result.is_ok(),
        "Call statement should be accepted: {:?}",
        result.err()
    );
}
...
"#;
    ...
    assert!(
        result.is_ok(),
        "Inline variable reference should parse: {:?}",
        result.err()
    );
...
"#;
    ...
    if result.is_ok() {
        ...
        eprintln!(
            "NOTE: Bracket escape needs implementation: {:?}",
            result.err()
        );
    }
}
