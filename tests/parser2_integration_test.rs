...
use pasta_rune::parser::{parse_file, parse_str, Expr, FileItem, LocalSceneItem};
use std::path::Path;
...
fn verify_pest_span_byte_offset_api() {
    ...
    // 基本的な妥当性チェック
    assert!(start_byte <= end_byte, "start_byte should be less than or equal to end_byte");
    assert!(end_byte <= source.len(), "end_byte should not exceed source length");
