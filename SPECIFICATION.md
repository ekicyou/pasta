...
**セマンティクス**: アクター辞書の定義またはアクタースコープの指定に使用します。詳細は [11. アクター辞書（Actor Dictionary）](#11-アクター辞書actor-dictionary) を参照。
...
- アクタースコープ内で Rune コードブロック（` ```rune ` ... ` ``` `）を配置可能（将来拡張用、現時点では使用例なし）
...
- `actor_name：` で始まる会話行では、そのアクターの単語辞書を優先的に参照する（詳細は [6. アクション（Action Line）](#6-アクションaction-line) を参照）
...
アクタースコープ内に Rune コードブロックを配置する構文は定義されていますが、現時点では使用例がなく、将来の拡張として予約されています。
