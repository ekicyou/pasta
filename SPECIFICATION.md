...
- 代替: グローバルラベルにチェイン用ラベルを定義し、`＞＊チェイン` などで呼び出してチェイン処理を行う。
- チェイン固有の出力や関数定義は、当該ラベル配下（暗黙 __start__ 含む）に記述する。

### 11.4 ローカルラベルのパラメータ（将来検討）
- グローバルラベル、ローカルラベルともにパラメータ付き構文は現時点で未定義。
...
- ファイルレベル属性はグローバルラベルに継承されるが、グローバルラベル側で同一属性が定義されれば上書き。
- ファイルレベル属性はローカルラベルには影響しない。
