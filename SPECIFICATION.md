...
#### パターン1: グローバルシーン参照
```text
call_target ::= グローバルシーンマーカー ~ シーン名
例: ＊会話
   *conversation
```

**セマンティクス**: 指定名で始まるグローバルシーンの `__start__` を呼び出す
...
**セマンティクス**: 現在のグローバルシーン内の指定ローカルシーンを呼び出す
...
**スコープ解決ルール**:
- 単語参照にスコープ修飾子は不要
- `＠word_name` で、現在のグローバルシーンスコープ内のローカル単語 + グローバル単語の全候補から検索
