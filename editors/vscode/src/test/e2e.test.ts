...
test('declares all 12 custom semantic token types', () => {
  ...
  const expected = ['scene', 'word', 'call', 'actor', 'actorName', 'talk', 'codeBlock', 'sakuraScript', 'escape', 'number', 'cueMarker', 'cueCommand'];
  for (const e of expected) {
    assert.ok(customTypes.includes(e), `should declare custom type '${e}'`);
  }
  assert.strictEqual(customTypes.length, expected.length, 'no unexpected extra custom token types');
});
...
test('has semantic token scopes for all 17 types', () => {
  const scopes = pkg.contributes.semanticTokenScopes[0].scopes;
  // Must match PASTA_TOKEN_TYPES in semanticTokensProvider.ts (indices 0-16)
  const expectedKeys = [
