# Pasta DSL Grammar Reference

Pasta DSLは、里々/さとりにインスパイアされた対話スクリプト記述言語です。このドキュメントでは、Pasta DSLの文法とその使用方法について説明します。

> **権威的仕様書**: 実装判断の根拠は [doc/spec/](doc/spec/) を参照してください。本ドキュメントは学習用のクイックリファレンスです。

## 目次

1. [基本構文](#基本構文)
2. [シーン定義](#シーン定義)
3. [アクション行（発言文）](#アクション行発言文)
4. [属性](#属性)
5. [変数](#変数)
6. [制御構文](#制御構文)
7. [単語定義と参照](#単語定義と参照)
8. [さくらスクリプト](#さくらスクリプト)
9. [Luaコードブロック](#luaコードブロック)
10. [コメント](#コメント)
11. [リテラル型](#リテラル型)
12. [キューコマンド行](#キューコマンド行)
13. [選択肢定義](#選択肢定義)

---

## 基本構文

### ファイル構造

Pastaスクリプトは**行指向文法**です。行頭の数文字により行属性が確定します。

```
ファイル
├─ グローバル単語定義 (＠)
├─ グローバルシーン (＊)
│   ├─ 属性行 (＆)
│   ├─ ローカル単語定義 (＠)
│   ├─ アクション行
│   ├─ 変数代入 (＄)
│   ├─ Call行 (＞)
│   ├─ キューコマンド行 (！)
│   ├─ 選択肢行 (＠？)
│   ├─ ローカルシーン (・)
│   └─ Luaブロック
└─ コメント行 (＃)
```

### マーカー一覧（全角/半角両対応）

| マーカー         | 全角 | 半角 | 用途                         |
| ---------------- | ---- | ---- | ---------------------------- |
| グローバルシーン | `＊` | `*`  | シーン定義                   |
| ローカルシーン   | `・` | `-`  | サブシーン定義               |
| 属性             | `＆` | `&`  | メタデータ                   |
| 単語/関数        | `＠` | `@`  | 単語定義・参照・関数呼び出し |
| 選択肢           | `＠？` | `@?` | 選択肢定義（ジャンプ先＋表示テキスト） |
| 変数             | `＄` | `$`  | 変数宣言・参照               |
| Call             | `＞` | `>`  | シーン呼び出し               |
| コメント         | `＃` | `#`  | コメント行                   |
| キューコマンド   | `！` | `!`  | 演出キュー（dola側で処理）   |
| コロン           | `：` | `:`  | 区切り                       |

### 空白

- 空白文字は、UnicodeのWhite_Spaceカテゴリの文字から、改行コード`\r``\n`を除いたものとします
- 半角スペース、全角スペース、タブなどが含まれます

### インデント

- シーン配下のステートメントは**インデント**（空白文字）で字下げします
- インデントレベルが一貫している必要は**ありません**
- **インデントの有無**のみで判定します（深さは判定不要）

---

## シーン定義

### グローバルシーン

グローバルシーンは、行頭が`＊`（全角アスタリスク）または`*`で始まります。

```pasta
＊挨拶
    ぱすた：こんにちは！
    ラザニア：ごきげんよう！
```

### ローカルシーン

ローカルシーンは、親シーン内でのみアクセス可能で、インデント＋`・`または`-`で始まります。

```pasta
＊メインメニュー
    ぱすた：何をしますか？
    ＞選択肢

    ・選択肢1
        ぱすた：選択肢1が選ばれました

    ・選択肢2
        ぱすた：選択肢2が選ばれました
```

### シーンの重複（前方一致によるランダム選択）

同じ名前のシーンを複数定義できます。実行時にランダムに1つが選択されます。

```pasta
＊挨拶
    ぱすた：おはよう！

＊挨拶
    ぱすた：こんにちは！

＊挨拶
    ぱすた：こんばんは！
```

シーン呼び出しは**前方一致**で候補を列挙します：

- `＞挨拶` では「挨拶」で始まるすべてのシーンが候補
- 候補からランダムに選択されます

### 予約パターン

`__*__` パターン（例：`__start__`）はシステム予約です。使用できません。

---

## アクション行（発言文）

### 基本形式

```
アクター名：アクション内容
```

例：
```pasta
＊会話
    ぱすた：今日はいい天気だね
    ラザニア：そうですわね、散歩に参りましょうか
```

### アクターの省略

一度アクターを指定すると、次の発言でアクターを省略できます。

```pasta
＊長い会話
    ぱすた：今日はね
    ：とてもいい天気なんだ
    ：だから散歩に行こうと思ってるんだ

    ラザニア：よろしくてよ！
    ：ご一緒いたしますわ！
```

### インライン要素

アクション行内に埋め込み可能な要素：

| 要素             | 構文                 | 説明                         |
| ---------------- | -------------------- | ---------------------------- |
| 通常テキスト     | `こんにちは`         | 任意の文字列                 |
| 単語参照         | `＠word_name`        | 登録済み単語からランダム選択 |
| ＠エスケープ     | `＠＠`               | リテラルの「＠」を1文字出力  |
| 変数参照         | `＄var_name`         | 変数値を展開                 |
| 関数呼び出し     | `＠func_name()`      | Lua関数を呼び出し            |
| Sakuraスクリプト | `\n`, `\w8`, `\s[0]` | 表情・タイミング制御         |

### 行継続

複数行にわたるアクション：

```pasta
＊長文
    ぱすた：長い台詞は
        ：複数行に分けて
        ：記述できます
```

継続行は行頭に`：`（コロン）を付けてインデント付きで記述します。行マーカー（＄＠＞＆＊・）で始めてはなりません。

> **重要**: 継続行は`：`で始まる必要があります。アクター名は指定しません。前行の発話を継続します。

---

## 属性

### 属性の設定

属性は、`＆属性名：値`の形式で設定します。**シーン定義の直後にのみ**記載できます。

```pasta
＊会話
    ＆author：Alice
    ＆genre：comedy
    ぱすた：こんにちは！
```

> **注意**: 属性の構文は現在パーサーで受け入れられていますが、トランスパイラーやランタイムでの処理実装は**将来予定**です。現在は属性を指定しても処理には反映されません。

### 配置ルール

- グローバルシーンの直後：グローバルシーンに属性を付与
- ローカルシーンの直後：ローカルシーンに属性を付与
- アクション行や変数代入行の後には配置**できません**

```pasta
＊選択肢
    ＆priority：high
    ・選択肢1
        ＆difficulty：3
        ぱすた：選択肢1です
```

---

## 変数

### 変数宣言と代入

変数は、`＄変数名：値`の形式で設定します。

```pasta
＊初期化
    ＄score：100
    ＄ユーザー：太郎
    ＄is_active：true
```

### 変数スコープ

| スコープ   | 宣言                | 参照            | 有効範囲                              |
| ---------- | ------------------- | --------------- | ------------------------------------- |
| ローカル   | `＄var_name＝値`    | `＄var_name`    | 一連のシーンが終わるまで              |
| グローバル | `＄＊var_name＝値`  | `＄＊var_name`  | 永続的                                |
| プロパティ | `＄％prop.path＝値` | `＄％prop.path` | SSP共有プロパティ（ベースウェア管理） |

### 変数参照

会話行の中で`＄変数名`の後に**空白を付けて**とすると、変数の内容が展開されます。

```pasta
＊表示
    ＄ユーザー：太郎
    ぱすた：＄ユーザー　さん、こんにちは！
```

> **重要**: 変数参照は`＄変数名`の直後に**1文字以上の空白**が必須です。空白がない場合は最長一致で変数名を解釈するため、意図しない動作になる可能性があります。

値を代入していない変数を参照した場合は、未定義単語と同じく**空文字列**として展開され、ログに警告メッセージが出力されます。

### 式（Expression）のサポート

**Pasta DSLでは式を記述できます**。変数代入、関数引数などで式を使用できます：

```pasta
# ✅ 許可される構文
＄score＝100                    # リテラル値
＄value＝＠word_name            # 単語参照
＄new_var＝＄old_var            # 変数参照
＄result＝＠calculate()         # 関数呼び出し
＄sum＝＄a + ＄b                 # 算術式
＄calc＝（＄x + ＄y）* 2        # 括弧による優先順位制御
```

### 対応演算子

| 種別 | 演算子（全角/半角） |
| ---- | ------------------- |
| 加算 | `+` / `＋`          |
| 減算 | `-` / `－`          |
| 乗算 | `*` / `＊` / `×`    |
| 除算 | `/` / `／` / `÷`    |
| 剰余 | `%` / `％`          |

複雑な演算や条件判定が必要な場合は、Luaブロックで関数を定義することも可能です。

詳細は [doc/spec/01-grammar-model.md](doc/spec/01-grammar-model.md) を参照。

---

## 制御構文

Pasta DSLは**宣言的言語**であり、命令型制御構文（if/while等）は含みません。コントロールフローは**Call文**と**シーン定義**で実現します。

### Call文（サブルーチン呼び出し）

Call文は他のシーンを呼び出し、実行後に元の場所に戻ります。

```pasta
＊メイン
    ぱすた：自己紹介を始めます
    ＞自己紹介
    ぱすた：自己紹介が終わりました

    ・自己紹介
        ぱすた：私はぱすたです
```

### Callターゲットの形式

| 形式           | 構文         | 説明                                                                   |
| -------------- | ------------ | ---------------------------------------------------------------------- |
| シーン参照     | `＞シーン名` | アクションスコープから参照できるローカル・グローバルシーン全てから選択 |
| 動的ターゲット | `＞＄変数名` | 変数値をシーン名として解決                                             |

```pasta
＊スタート
    ＞次のステップ              # シーン参照（ローカル・グローバルシーンから選択）

    ・次のステップ
        ぱすた：次のステップに来ました
```

### 条件分岐の実現

条件分岐はLuaブロック内で実装し、`act:call()` を使用して分岐します。

````pasta
＊メイン
    ＄スコア：75

```lua
function SCENE.分岐判定(act)
    local save, var = act:init_scene(SCENE)
    local スコア = var.スコア
    local label
    if スコア >= 90 then
        label = "優秀"
    elseif スコア >= 50 then
        label = "合格"
    else
        label = "不合格"
    end
    return act:call(SCENE.__global_name__, label, {})
end
```

    ＞＠分岐判定

    ・優秀
        ぱすた：優秀です！

    ・合格
        ぱすた：合格です

    ・不合格
        ぱすた：不合格です
````

### チェイントーク（継続トーク）

チェイントーク（継続トーク）は、1つのシーンの出力を途中で分割し、複数回に分けて返す機能です。`＞チェイントーク` または `＞yield` と記述するだけで利用できます。

```pasta
＊ランダムトーク
    ぱすた：今日はいい天気ですね
    ＞チェイントーク
    ぱすた：明日も晴れるといいな
```

**動作**:
- `＞チェイントーク` の前に蓄積されたトークンが1回目の出力（中間出力）として返される
- `＞チェイントーク` の後のトークンは2回目の出力（最終出力）として返される
- `＞yield` も同じ動作（エイリアス）

**仕組み**: GLOBAL テーブルに事前登録された `チェイントーク` / `yield` 関数が `act:yield()` を呼び出し、コルーチンの `coroutine.yield()` でシーン実行を中断します。`ACT_IMPL.call` の4段階検索において Level 3（GLOBAL テーブル）で解決されます。

> **カスタマイズ**: `main.lua` 等で `GLOBAL.チェイントーク` を再定義することで、独自のチェイントーク動作を実装できます。

---

## 単語定義と参照

### 概要

単語定義は、テキストのバリエーションをランダムに選択するための機能です。定義された単語群からランダムに1つを選択することで、会話に自然な多様性を持たせることができます。

### グローバル単語定義

ファイルの先頭（シーン定義の前）にインデントなしで`＠単語キー：値1、値2、値3`形式で登録します。グローバル単語定義はファイル全体から参照可能です。値は読点（、）またはカンマ（，/,）で区切ります。

```pasta
# グローバル単語定義（ファイル先頭）
＠挨拶朝：おはよう、ハロー、Good morning
＠挨拶昼：こんにちは

＊メイン
    ぱすた：＠挨拶朝　！
```

### 複数キー単語定義

コロンの左側にカンマ区切りで**複数のキー**を指定できます。同一の値リストを異なるキー名すべてに同時登録します。辞書の重複記述を削減できます。

```pasta
# 複数キー指定 — 「女性」でも「水の妖精」でも同じ候補が返る
＠女性、水の妖精：水無灯里、アリス・キャロル

# 3キー以上も可能
＠挨拶、あいさつ、greeting：こんにちは、ハロー
```

- キー区切りには全角読点（`、`）・全角コンマ（`，`）・半角カンマ（`,`）が使えます
- グローバル・ローカル・アクタースコープのいずれでも使用可能です
- 単一キー形式（従来の`＠key：values`）との後方互換性は維持されています

### ローカル単語定義

グローバルシーン内でインデントありで登録します。**シーンの先頭**に配置し、会話行より前に定義してください。

```pasta
＊メイン
    ＠挨拶朝：Hello、Good morning
    ぱすた：＠挨拶朝　！
```

> **Note**: ローカル単語定義は、そのグローバルシーン内でのみ参照可能です。

### スコープと優先順位

単語参照時の検索順序：

1. **ローカル単語辞書** - 現在のシーン内で定義された単語
2. **グローバル単語辞書** - ファイル先頭で定義された単語

同名の単語がローカルとグローバルの両方に存在する場合、**マージ**されて候補プールが統合されます。

```pasta
＠挨拶：グローバル挨拶

＊メイン
    ＠挨拶：ローカル挨拶
    ぱすた：＠挨拶　！  # 「グローバル挨拶」または「ローカル挨拶」が選ばれる
```

### 単語の呼び出し（インライン参照）

会話行の中で`＠単語キー`の後に**空白を付けて**とすると、一致する単語からランダムに1つ選択されます。

```pasta
＠挨拶朝：おはよう、ハロー
＠挨拶昼：こんちわ
＊メイン
    ＠挨拶朝：Hello
    ぱすた：＠挨拶昼　。
    ラザニア：＠挨拶　。
```

- `＠挨拶昼　` → 候補は「こんちわ」のみ
- `＠挨拶　` → 前方一致により候補は「おはよう、ハロー、こんちわ、Hello」

> **重要**: 単語参照は`＠単語キー`の直後に**1文字以上の空白**が必須です。空白がない場合は最長一致で単語キーを解釈するため、意図しない動作になる可能性があります。

### 前方一致検索

単語参照は**前方一致**で候補を検索します。これにより、関連する単語群を一括で参照できます。

```pasta
＠挨拶：こんにちは、ハロー
＠挨拶_朝：おはよう、グッドモーニング
＠挨拶_夕：こんばんは、Good evening

＊会話
    ぱすた：＠挨拶　       # 全9候補からランダム選択
    ぱすた：＠挨拶_朝　    # 「おはよう」「グッドモーニング」から選択
    ぱすた：＠挨拶_夕　    # 「こんばんは」「Good evening」から選択
```

前方一致の動作：
- `＠挨` → 「挨拶」「挨拶_朝」「挨拶_夕」すべてにマッチ
- `＠挨拶_朝` → 「挨拶_朝」のみにマッチ

### Call文と単語参照の分離

**重要**: Call文（`＞`）は**シーン辞書のみ**を検索し、単語辞書は参照しません。同様に、単語参照（`＠`）はシーン辞書を参照しません。

```pasta
＠挨拶：こんにちは  # 単語定義

＊挨拶              # シーン定義
    ぱすた：やあ！

＊メイン
    ぱすた：＠挨拶　   # 単語辞書から「こんにちは」を取得
    ＞挨拶           # シーン参照（「挨拶」シーンを呼び出し）
```

### 未定義単語の参照

存在しない単語キーを参照した場合、**空文字列**が返されます（エラーにはなりません）。ログに警告メッセージが出力されます。

```pasta
＊テスト
    ぱすた：＠存在しない単語　  # 空文字列として展開される
```

### ＠エスケープ

`＠＠`と書くと、リテラルの「＠」が1文字出力されます。

```pasta
＊メール
    ぱすた：メールは user＠＠example.com 　です
```

### 引用符エスケープ

単語値に引用符を含める場合は、二重の括弧を使用します：

```pasta
＠会話：「「セリフ」」  # 実行時に「セリフ」として展開される
```

- 外側の`「」`は単語値の区切り
- 内側の`「」`はリテラルの引用符として保持される

---

## さくらスクリプト

Pastaは、さくらスクリプトのエスケープシーケンスを**そのまま**IR出力に含めます。解釈はareka層が行います。

**エスケープは半角のみ**：
- バックスラッシュ: `\`
- 括弧: `[` と `]`

### 字句構造

```
sakura_command   ::= "\" ~ sakura_token ~ bracket_content?
sakura_token     ::= [!\-+*?&_a-zA-Z0-9]+
bracket_content  ::= "[" ~ bracket_chars ~ "]"
bracket_chars    ::= ( "\\]" | [^\]] )*
```

### 使用例

```pasta
＊表情変更
    ぱすた：\s[0]こんにちは！
    ぱすた：\s[4]元気だよ
    ぱすた：\s[1]\w8それで\w8ね...

＊改行とウェイト
    ぱすた：最初の行\n次の行\n\w9最後の行

＊ブラケットエスケープ
    ぱすた：配列参照\s[a\]b]
```

### 角括弧内のエスケープ

- `\]` で `]` を文字として含める
- カンマを含む値はダブルクォートで囲む: `\![raise,OnTest,"100,2"]`
- ダブルクォートを含める場合は二重にする: `\![call,ghost,"the ""Name"""]`

---

## Luaコードブロック

Luaブロックを使用して、ローカル関数を定義できます。

### 構文

````pasta
＊シーン名
```lua
function SCENE.関数名(act)
    local save, var = act:init_scene(SCENE)
    -- Luaコード
end
```
````

または `lua` 接尾辞なし:

````pasta
＊シーン名
```
function SCENE.関数名(act)
    local save, var = act:init_scene(SCENE)
    -- Luaコード
end
```
````

### 制約

- **関数定義のみ許可**: `function ...` で始まる関数定義のみ記述可能
- **ステートメント不可**: `local x = 10` などの直接的なステートメントは許可されません
- **インデント不要**: Luaブロックはインデントなしで記述可能（直前のグローバルシーンに属します）

### 例

````pasta
＊計算
```lua
function SCENE.calculate(act)
    local save, var = act:init_scene(SCENE)
    local result = 10 + 20
    return result
end

function SCENE.greet(act)
    local save, var = act:init_scene(SCENE)
    act.ぱすた:talk("こんにちは！")
end
```
    ぱすた：計算結果は＠calculate()　です
````

---

## コメント

### 行コメント

`#` または `＃` から行末までがコメントとして扱われます。

```pasta
＊テスト
    # これはコメントです
    ぱすた：これは実行されます
    ＃ 全角シャープでもOK
```

コメント行はあらゆる位置に配置可能で、構造・セマンティクスに影響しません。

---

## リテラル型

### 型変換ルール

リテラル値は以下の優先順位で型変換されます：

| 優先度 | 値               | 型                   |
| ------ | ---------------- | -------------------- |
| 1      | `true` / `false` | bool                 |
| 2      | `「...」`        | String（引用符あり） |
| 3      | 小数点を含む数値 | f64                  |
| 4      | 小数点なしの数値 | i64                  |
| 5      | その他           | String               |

### 文字列リテラル

```pasta
# 日本語文字列
＄greeting：「こんにちは」

# 英語文字列
＄message："Hello, World!"
```

空白を含む文字列は引用符で囲む必要があります。

---

## キューコマンド行

### 概要

キューコマンド行は、演出制御（表情切替、BGM、ウェイト、選択肢など）をDSLレベルで記述するための仕組みです。行頭（インデント後）に`!`または`！`（全角感嘆符）を記述して開始します。

キューコマンドはパーサーでAST化されますが、Lua変換時にはスキップされます（dola側で処理される想定）。

### 基本構文

```
!コマンド名
!コマンド名@スコープ名
!コマンド名(引数1, 引数2, ...)
!コマンド名@スコープ名(引数1, 引数2, ...)
```

### 全角/半角対応

マーカーおよび括弧は全角/半角のいずれでも使用できます。

```pasta
!clear              # 半角マーカー
！clear              # 全角マーカー
！選択待ち（30）     # 全角マーカー＋全角括弧
!emote@笑顔(smile)  # 半角
```

### スコープ指定

`@`に続けてスコープを指定できます。`アクター名:スコープ名`の形式でアクターを明示できます。

```pasta
!emote@笑顔           # スコープ: name="笑顔", actor=なし
!emote@さくら:笑顔     # スコープ: name="笑顔", actor="さくら"
```

### 引数

括弧`()`内にカンマ区切りで引数を記述します。以下のトークン型を使用できます。

| 型         | 例                       | 説明          |
| ---------- | ------------------------ | ------------- |
| 識別子     | `normal`, `さくら`       | Unicode識別子 |
| 文字列     | `「はい、行きましょう」` | 日本語文字列  |
| 整数       | `30`, `-5`               | 整数リテラル  |
| 浮動小数点 | `10.0`, `-5.5`           | 浮動小数点数  |
| @参照      | `@名前`                  | 単語参照      |

```pasta
!yield(10.0)
!select(30)
!choice@はい(yes, 「はい、行きましょう！」)
!seek(@名前, 1.0)
```

### シーン内での使用例

```pasta
＊起動挨拶
    !emote@普通(normal)
    さくら：こんにちは！
    !wait@さくら(500)
    ！bgm(morning.ogg)
    うにゅう：やあ！
    !yield(10.0)
    !clear
```

> **注意**: キューコマンド行はローカルシーン内のアイテムとして扱われます。Lua変換ではスキップされますが、ASTには `CueCommandNode` として保持されます。

---

## 選択肢定義

### 概要

選択肢行は、プレイヤーに選択肢を提示するための構文です。`＠？`（または`@?`）マーカーで開始し、ジャンプ先のシーン名と任意の表示テキストを指定します。さくらスクリプトの`\q[...]`タグが自動生成され、選択結果のルーティングも自動化されます。

### 基本構文

#### 省略形（表示テキスト＝ターゲット名）

```pasta
＠？挨拶
```

`挨拶` がジャンプ先シーン名かつ表示テキストになります。

#### 括弧形（表示テキスト指定）

```pasta
＠？挨拶「挨拶する」
```

`挨拶` がジャンプ先、`挨拶する` が表示テキストになります。

### 全角/半角対応

全角 `＠？` と半角 `@?` は等価です。

```pasta
＠？挨拶
@?greeting
```

### 選択肢タイムアウト

`!select(秒数)` キューコマンドで選択の制限時間を設定します。

```pasta
!select(30)        # 30秒でタイムアウト
```

引数省略時はタイムアウトなし（無制限待機）です。

### 自動ルーティング

選択肢をクリックすると `OnChoiceSelectEx` イベントが発生し、選択IDでシーンを前方一致検索して自動実行します。ローカルシーン → グローバルシーンの順で検索されます。

### 使用例

```pasta
＊メニュー
    ぱすた：何をしますか？
    ＠？挨拶「挨拶する」
    ＠？自己紹介
    !select(30)

    ・挨拶
        ぱすた：こんにちは！
    ・自己紹介
        ぱすた：私はぱすたです。
```

---

## エラーハンドリング

### パースエラー

文法エラーが発生すると、行番号とカラム位置を含むエラーが返されます。

```
Parse error at <script>:5:10: Expected ':' after speaker name
```

### シーン未発見エラー

存在しないシーンを実行しようとすると、`LabelNotFound`エラーが返されます。

```
Label not found: 存在しないシーン名
```

### ランタイムエラー

実行中のエラーは`ScriptEvent::Error`として yield されます。

---

## 参考資料

- [doc/spec/](doc/spec/) - 権威的仕様書（章別分割、12章）
- [サンプルスクリプト](crates/pasta_lua/tests/fixtures/)
- [pasta-lua-coding skill](.agents/skills/pasta-lua-coding/SKILL.md) - Lua APIリファレンス（references/に詳細）

---

**Pasta DSL** - 里々/さとりにインスパイアされた、シンプルで強力な対話スクリプト言語
