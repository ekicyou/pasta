...
グローバルラベルは、行頭が`＊`（全角アスタリスク）または`*`で始まります。
...
ローカルラベルは、親ラベル内でのみアクセス可能で、インデント＋`・`または`-`で始まります。
...
ラベル呼び出しは**前方一致**で候補を列挙します：

- `＞挨拶` では「挨拶」で始まるすべてのラベルが候補
- 候補からランダムに選択されます
...
属性は、`＆属性名：値`の形式で設定します。**ラベル定義の直後にのみ**記載できます。
...
| スコープ | 宣言 | 参照 | 有効範囲 |
|---------|------|------|---------|
| ローカル | `＄var_name：値` | `＄var_name` | 親ラベル内 |
| グローバル | `＄＊var_name：値` | `＄＊var_name` | ファイル全体 |
...
**Pasta DSLでは式を記述できません**。代入可能な値は以下のみです：
...
複雑な演算が必要な場合は、Runeブロックで関数を定義してください。

---

## 制御構文

Pasta DSLは**宣言的言語**であり、命令型制御構文（if/while等）は含みません。コントロールフローは**Call文**と**ラベル定義**で実現します。
...
| 形式 | 構文 | 説明 |
|------|------|------|
| ローカルラベル | `＞ラベル名` | 現在のグローバルラベル内のローカルラベル |
| グローバルラベル | `＞＊ラベル名` | 別のグローバルラベル |
| 動的ターゲット | `＞＄変数名` | 変数値をラベル名として解決 |

```pasta
＊スタート
    ＞＊次のステップ        # グローバルラベルへのCall
...
ファイルの先頭（ラベル定義の前）にインデントなしで`＠単語キー：値1　値2　値3`形式で登録します。グローバル単語定義はファイル全体から参照可能です。
...
グローバルラベル内でインデントありで登録します。**ラベルの先頭**に配置し、会話行より前に定義してください。
...
> **Note**: ローカル単語定義は、そのグローバルラベル内でのみ参照可能です。

### スコープと優先順位

単語参照時の検索順序：

1. **ローカル単語辞書** - 現在のラベル内で定義された単語
...
**重要**: Call文（`＞`）は**ラベル辞書のみ**を検索し、単語辞書は参照しません。同様に、単語参照（`＠`）はラベル辞書を参照しません。
...
- **インデント不要**: Runeブロックはインデントなしで記述可能（直前のグローバルラベルに属します）
